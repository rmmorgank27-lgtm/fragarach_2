...
public final class SQLiteReadService: @unchecked Sendable {
    ...
    public func load(path: String, operationLimit: Int = 100) throws -> AuthoritySnapshot {
        ...
        guard tables == foundationTables else { throw AuthorityReadError.incompatibleDatabase("exact seven-table identity failed") }
        let migrations = try int(db, "SELECT count(*) FROM schema_migrations")
        guard migrations == 3 else { throw AuthorityReadError.incompatibleDatabase("expected three recognized migrations") }
