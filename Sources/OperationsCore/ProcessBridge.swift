...
public enum ArgumentBuilder {
    public static func arguments(for intent: OperationIntent, database: String) -> [String] {
        ...
        case .configureProvider(let provider,let enabled,let operationalLimit,let concurrencyLimit): ["-m","fragarach_ii.commands.provider_settings","--mode","update","--provider",provider,"--enabled",enabled ? "true":"false","--operational-limit","\(operationalLimit)","--concurrency-limit","\(concurrencyLimit)","--json"]
        case .configureProviderRoute(let provider,let asset,let providerSymbol,let timeframe,let mappingClass,let calendarID): ["-m","fragarach_ii.commands.provider_routes","--mode","upsert","--provider",provider,"--asset",asset,"--provider-symbol",providerSymbol,"--timeframe",timeframe,"--mapping-class",mappingClass,"--calendar-id",calendarID,"--json"]
