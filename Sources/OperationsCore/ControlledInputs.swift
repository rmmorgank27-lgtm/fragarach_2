...
public struct ReviewedDataOperationPlan:Identifiable,Equatable,Sendable {
    ...
    public let from:String?;public let through:String?;public let sourceTimezone:String?;public let conflict:ConflictMode
    public init(id:UUID,mode:DataOperationsMode,instrument:String,timeframe:String,filePath:String?=nil,fileChecksum:String?=nil,fileSelectionID:UUID?=nil,from:String?=nil,through:String?=nil,sourceTimezone:String?=nil,conflict:ConflictMode){self.id=id;self.mode=mode;self.instrument=instrument;self.timeframe=timeframe;self.filePath=filePath;self.fileChecksum=fileChecksum;self.fileSelectionID=fileSelectionID;self.from=from;self.through=through;self.sourceTimezone=sourceTimezone;self.conflict=conflict}
    public var intent:OperationIntent? { switch mode { case .importFile: guard let filePath else{return nil};return .importCSV(file:filePath,symbol:instrument,timeframe:timeframe,sourceTimezone:sourceTimezone,mode:conflict);case .fetch:guard let from,let through else{return nil};return .acquire(asset:instrument,timeframe:timeframe,from:from,through:through,mode:conflict);default:return nil } }
