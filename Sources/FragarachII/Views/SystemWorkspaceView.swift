...
struct SystemWorkspaceView:View {
    ...
    var body:some View { VStack(alignment:.leading,spacing:14) {
        ...
        Picker("Section",selection:$store.systemSection){ForEach(SystemSection.allCases){Text($0.rawValue)}}.pickerStyle(.segmented).frame(maxWidth:620)
        Group { switch store.systemSection { case .status:SystemStatusView();case .backups:IntegrityBackupView();case .settings:DiagnosticsSettingsView();case .audit:AuthorityLedgerView() } }.frame(maxWidth:.infinity,maxHeight:.infinity)
    }.padding() }
}
