...
private enum FetchIntent: String, CaseIterable, Hashable { case initial = "Fetch Initial History", update = "Update", custom = "Custom Range" }
...
struct DataOperationsView: View {
    ...
    private var selectableTimeframes:[String]{capability?.timeframes.filter{$0.policyState=="REQUIRED" && $0.authorityState != "BLOCKED"}.map(\.timeframe) ?? ["D1"]}
    ...
    private func fetchView(_ r:InstrumentRegistrationRecord)->some View { VStack(alignment:.leading,spacing:14) {
        laneMatrix(r)
        Picker("Timeframe",selection:$selectedTimeframe){ForEach(capability?.timeframes.filter{$0.policyState=="REQUIRED" && $0.authorityState != "BLOCKED"} ?? [],id:\.timeframe){item in Text(item.timeframe).tag(item.timeframe)}}.pickerStyle(.segmented)
        ...
        if intent == .initial { Label("Provider resolution requests bounded \(selectedTimeframe) authority using its exact contract.",systemImage:"clock.arrow.circlepath").foregroundStyle(.secondary) }
    ...
    private func laneMatrix(_ r:InstrumentRegistrationRecord)->some View { GroupBox("Timeframe Lane Matrix") { Grid(alignment:.leading,horizontalSpacing:18,verticalSpacing:8) { GridRow{ForEach(["Timeframe","Policy","Authority","Evidence","Truth","Selectable"],id:\.self){Text($0).font(.caption.bold())}};Divider().gridCellColumns(6);ForEach(capability?.timeframes ?? []){item in GridRow{Text(item.timeframe).monospaced();Text(item.policyState);Text(item.authorityState);Text(item.evidenceState);Text(item.truthState);Text(item.policyState=="REQUIRED" && item.authorityState != "BLOCKED" && !r.retired ? "Yes":"No")}} }.frame(maxWidth:.infinity,alignment:.leading) } }

    @ViewBuilder private var readableResult:some View {
        if let plan=completedPlan,plan.matches(mode:store.dataOperationsMode,instrument:registration?.asset,timeframe:selectedTimeframe,fileChecksum:file == nil ? nil:checksum),let owned=store.currentOperationResult,owned.planRevision==plan.id {
    ...
    private func readableFacts(_ json:[String:Any])->[(String,String)] {
        let common=[("Instrument",json["asset"] as? String ?? registration?.asset ?? "—"),("Timeframe",json["timeframe"] as? String ?? selectedTimeframe)]
    ...
    @ViewBuilder private var reviewSheet:some View { if let plan=reviewedPlan { VStack(alignment:.leading,spacing:16) { Text(plan.mode == .importFile ? "Review Import":"Review Data Operation").font(.title);Facts([("Instrument",plan.instrument),("Source",plan.mode == .importFile ? "Manual file import":"Automatic provider resolution"),("Lane",plan.timeframe),("Intent",plan.mode == .importFile ? "IMPORT_FILE":intent.rawValue),("Requested range",plan.mode == .importFile ? URL(fileURLWithPath:plan.filePath ?? "").lastPathComponent:"\(plan.from ?? "—") → \(plan.through ?? "—") inclusive"),("Source timezone",plan.mode == .importFile ? plan.sourceTimezone ?? "Explicit offsets in source":"Not applicable"),("File checksum",plan.fileChecksum ?? "—"),("Conflict Policy",plan.conflict.rawValue.capitalized)]);HStack{Button("Cancel",role:.cancel){reviewing=false;reviewedPlan=nil};Spacer();Button(plan.mode == .importFile ? "Confirm Import":"Run Data Operation"){runReviewed(plan)}.buttonStyle(.borderedProminent)} }.padding(24).frame(minWidth:620) } }
    ...
    private func resetInstrumentContext(){if !selectableTimeframes.contains(selectedTimeframe){selectedTimeframe="D1"};throughDate=latestCompletedBoundary;if (lane?.barCount ?? 0)==0{intent = .initial;fromDate=Calendar.current.date(byAdding:.day,value:-(min(maximumDays,30)-1),to:throughDate)!}else if let latest=lane?.latestBar{intent = .update;fromDate=reconciliationStart(latest)}else{intent = .custom;fromDate=Calendar.current.date(byAdding:.day,value:-29,to:throughDate)!};file=nil;sourceTimezone="";reviewing=false;retirementImpact=nil;localError=nil;dateInterpretation=nil;conflict = .preserve;isolateOperationState()}
    ...
    private func prepareReview(_ r:InstrumentRegistrationRecord){store.clearCurrentOperationResult();let reviewedTimezone=sourceTimezone.trimmingCharacters(in:.whitespacesAndNewlines);let plan=ReviewedDataOperationPlan(id:store.currentPlanRevision,mode:store.dataOperationsMode,instrument:r.asset,timeframe:selectedTimeframe,filePath:file?.path,fileChecksum:file == nil ? nil:checksum,fileSelectionID:store.dataOperationsMode == .importFile ? fileSelectionID:nil,from:store.dataOperationsMode == .fetch ? dateRange.fromISO:nil,through:store.dataOperationsMode == .fetch ? dateRange.throughISO:nil,sourceTimezone:reviewedTimezone.isEmpty ? nil:reviewedTimezone,conflict:conflict);reviewedPlan=plan;completedPlan=nil;reviewing=true}
