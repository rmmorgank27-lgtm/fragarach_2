# CRYPTO M30 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority
**Authority Layer:** Market Timeframe
**Authority Name:** `CRYPTO_M30_AUTHORITY_V1`
**Market Name:** Cryptoassets
**Market Code:** `CRYPTO`
**Timeframe:** `M30`
**Version:** 1.0
**Status:** APPROVED
**Repository Location:** `constitution/authorities/crypto/CRYPTO_M30_AUTHORITY_V1.md`
...
This authority defines the approved operational truth for `M30` evidence within the Cryptoasset market ecosystem of Fragarach II.

It establishes:

- what one crypto `M30` bar represents;
...
- the `CRYPTO_M30_VALIDATOR_V1` validation contract;
...
One canonical crypto `M30` bar covers the fixed interval `[open, open + 30 minutes)` aligned to minute 00 or minute 30 of each UTC hour.

## 5.3 Bar Meaning

A complete crypto `M30` bar contains the approved provider's declared OHLC for one canonical interval within one declared venue or aggregate scope and one declared price basis.
...
Every canonical `M30` interval has a fixed elapsed duration of 30 minutes. Daylight-saving transitions do not change it.
...
`M30` intervals align to minute 00 or minute 30 of each UTC hour.
...
```text
interval_open = floor_UTC(timestamp, 30 minutes)
interval_end  = interval_open + 30 minutes
```
...
A canonical `M30` interval MAY cross regional session labels and exchange-local dates, but MUST NOT cross its UTC-aligned boundary.
...
Every accepted canonical `M30` bar MUST be identified by the UTC instant at which its interval begins.
...
- timestamp does not align to the canonical `M30` grid;
...
For one canonical `M30` interval and one compatible evidence identity:
...
Twelve Data `30min` labels MUST map to aligned UTC interval opens.
...
|---|---|---|---|
| `M5` | YES | Exactly 6 aligned, closed, compatible contributors; one provider/scope/price basis/effective segment | Full provenance required |
| Any higher timeframe | NO | Downsampling cannot construct a lower timeframe | Prohibited |

## 10.4 Direct Provider Authority

Twelve Data `30min` bars are approved as direct crypto `M30` evidence only when:
...
- `CRYPTO_M30_VALIDATOR_V1` passes.
...
Direct Twelve Data `30min` evidence is the primary automated acquisition source.

Complete derived `M30` evidence MAY fill uncovered intervals or provide verification, but MUST NOT silently replace conflicting direct evidence.
...
Complete eligible `M30` evidence MAY support construction of approved higher timeframes, subject to those authorities.
...
At `14:29:59Z`, the 14:00 M30 interval is open. At `14:30:00Z`, it is logically closed.
...
The latest expected closed `M30` interval open is:

```text
floor_UTC(current instant, 30 minutes) minus 30 minutes
```
...
| `end_date` | Desired final canonical interval-open datetime plus 30 minutes in UTC |
...
end_date   = E + 30 minutes represented in UTC
```
...
An incremental request SHOULD begin at least `96` expected `M30` intervals before the latest accepted closed interval and continue through the interval immediately after the latest expected closed open time, followed by canonical filtering.
...
There is no universal crypto `M30` start date.
...
4. Twelve Data `/earliest_timestamp` result for the registered symbol and `30min`;
...
| Derived evidence | Latest source-lane effective start rounded to the first fully covered `M30` interval | Latest fully covered target interval | Source authorities and complete compatible contributors required |
...
48 canonical `M30` intervals are expected per complete UTC day.
...
|---|---:|---|
| `FRONTIER` | 1–96 expected intervals | Highest repair priority and prominent warning |
| `RECENT` | 97–1440 expected intervals | Repair priority and visible warning |
| `HISTORICAL` | More than 1440 expected intervals | Retain, report, and repair proportionately; do not suppress usable newer history |
...
A crypto `M30` OHLC bar is evidence for its declared provider, scope, quote asset, and price basis.
...
**Validator:** `CRYPTO_M30_VALIDATOR_V1`
...
- minute is 00 or 30 and second/subsecond are zero;
...
| 2 | Twelve Data direct `30min` | Primary automated acquisition and uncovered-interval source | Contract, mapping, scope, range, and validation pass |
| 3 | Operator-supplied direct `M30` file | Manual backfill or supplementary source | Manifest, checksum, timezone, timestamp meaning, scope, quote asset, price basis, and validation required |
| 4 | Complete derived `M30` evidence | Uncovered-interval or verification source | Section 10 permits; one provider/scope/price basis and complete contributors |
...
- `CRYPTO_M30_AUTHORITY_V1` is approved;
...
- `CRYPTO_M30_VALIDATOR_V1` exists and is deterministic;
...
- align crypto `M30` bars to New York, London, exchange-local midnight, or daylight-saving time;
...
- `M30` duration, alignment, or ownership;
- canonical timestamp meaning;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial crypto `M30` timeframe authority drafted | Establish constitutional authority before implementation | PENDING |
...
- `M30` interval and UTC ownership are accepted;
...
- `CRYPTO_M30_VALIDATOR_V1` is accepted;
...
> `CRYPTO_M30_AUTHORITY_V1` is the approved constitutional authority for crypto `M30` evidence in Fragarach II. Every subordinate specification, implementation, acquisition, validation, construction, migration, evidence-lane operation, and acceptance proof MUST conform to it.
...
> Crypto `M30` authority is continuous, UTC-owned, identity-sensitive, scope-explicit, immutable, and non-blocking.
