# CRYPTO H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority
**Authority Layer:** Market Timeframe
**Authority Name:** `CRYPTO_H1_AUTHORITY_V1`
**Market Name:** Cryptoassets
**Market Code:** `CRYPTO`
**Timeframe:** `H1`
**Version:** 1.0
**Status:** DRAFT FOR APPROVAL
**Repository Location:** `constitution/authorities/crypto/CRYPTO_H1_AUTHORITY_V1.md`
...
This authority defines the approved operational truth for `H1` evidence within the Cryptoasset market ecosystem of Fragarach II.

It establishes:

- what one crypto `H1` bar represents;
...
- the `CRYPTO_H1_VALIDATOR_V1` validation contract;
...
One canonical crypto `H1` bar covers the fixed interval `[open, open + 60 minutes)` aligned to the top of each UTC hour.

## 5.3 Bar Meaning

A complete crypto `H1` bar contains the approved provider's declared OHLC for one canonical interval within one declared venue or aggregate scope and one declared price basis.
...
Every canonical `H1` interval has a fixed elapsed duration of 60 minutes. Daylight-saving transitions do not change it.
...
`H1` intervals align to the top of each UTC hour.
...
```text
interval_open = floor_UTC(timestamp, 60 minutes)
interval_end  = interval_open + 60 minutes
```
...
A canonical `H1` interval MAY cross regional session labels and exchange-local dates, but MUST NOT cross its UTC-aligned boundary.
...
Every accepted canonical `H1` bar MUST be identified by the UTC instant at which its interval begins.
...
- timestamp does not align to the canonical `H1` grid;
...
For one canonical `H1` interval and one compatible evidence identity:
...
Twelve Data `1h` labels MUST map to aligned UTC interval opens.
...
|---|---|---|---|
| `M30` | YES | Exactly 2 aligned, closed, compatible contributors; one provider/scope/price basis/effective segment | Full provenance required |
| `M5` | YES | Exactly 12 aligned, closed, compatible contributors; one provider/scope/price basis/effective segment | Full provenance required |
| Any higher timeframe | NO | Downsampling cannot construct a lower timeframe | Prohibited |

## 10.4 Direct Provider Authority

Twelve Data `1h` bars are approved as direct crypto `H1` evidence only when:
...
Direct Twelve Data `1h` evidence is the primary automated acquisition source.

Complete derived `H1` evidence MAY fill uncovered intervals or provide verification, but MUST NOT silently replace conflicting direct evidence.
...
Complete eligible `H1` evidence MAY support construction of approved higher timeframes, subject to those authorities.
...
At `14:59:59Z`, the 14:00 H1 interval is open. At `15:00:00Z`, it is logically closed.
...
The latest expected closed `H1` interval open is:

```text
floor_UTC(current instant, 60 minutes) minus 60 minutes
```
...
| `end_date` | Desired final canonical interval-open datetime plus 60 minutes in UTC |
...
end_date   = E + 60 minutes represented in UTC
```
...
An incremental request SHOULD begin at least `48` expected `H1` intervals before the latest accepted closed interval and continue through the interval immediately after the latest expected closed open time, followed by canonical filtering.
...
There is no universal crypto `H1` start date.
...
4. Twelve Data `/earliest_timestamp` result for the registered symbol and `1h`;
...
| Derived evidence | Latest source-lane effective start rounded to the first fully covered `H1` interval | Latest fully covered target interval | Source authorities and complete compatible contributors required |
...
24 canonical `H1` intervals are expected per complete UTC day.
...
|---|---:|---|
| `FRONTIER` | 1–48 expected intervals | Highest repair priority and prominent warning |
| `RECENT` | 49–720 expected intervals | Repair priority and visible warning |
| `HISTORICAL` | More than 720 expected intervals | Retain, report, and repair proportionately; do not suppress usable newer history |
...
A crypto `H1` OHLC bar is evidence for its declared provider, scope, quote asset, and price basis.
...
**Validator:** `CRYPTO_H1_VALIDATOR_V1`
...
- minute, second, and subsecond are zero;
...
| 2 | Twelve Data direct `1h` | Primary automated acquisition and uncovered-interval source | Contract, mapping, scope, range, and validation pass |
| 3 | Operator-supplied direct `H1` file | Manual backfill or supplementary source | Manifest, checksum, timezone, timestamp meaning, scope, quote asset, price basis, and validation required |
| 4 | Complete derived `H1` evidence | Uncovered-interval or verification source | Section 10 permits; one provider/scope/price basis and complete contributors |
...
- `CRYPTO_H1_AUTHORITY_V1` is approved;
...
- `CRYPTO_H1_VALIDATOR_V1` exists and is deterministic;
...
- align crypto `H1` bars to New York, London, exchange-local midnight, or daylight-saving time;
...
- `H1` duration, alignment, or ownership;
- canonical timestamp meaning;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial crypto `H1` timeframe authority drafted | Establish constitutional authority before implementation | PENDING |
...
- `H1` interval and UTC ownership are accepted;
...
- `CRYPTO_H1_VALIDATOR_V1` is accepted;
...
> `CRYPTO_H1_AUTHORITY_V1` is the approved constitutional authority for crypto `H1` evidence in Fragarach II. Every subordinate specification, implementation, acquisition, validation, construction, migration, evidence-lane operation, and acceptance proof MUST conform to it.
...
> Crypto `H1` authority is continuous, UTC-owned, identity-sensitive, scope-explicit, immutable, and non-blocking.
