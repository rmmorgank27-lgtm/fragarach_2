# AUSTRALIAN EQUITIES M5 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `AUSTRALIAN_EQUITIES_M5_AUTHORITY_V1`  
**Market Name:** Australian Equities  
**Market Code:** `EQUITIES_AU`  
**Timeframe:** `M5`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/equities_au/AUSTRALIAN_EQUITIES_M5_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `M5` evidence within the Australian Equities market ecosystem of Fragarach II.

It establishes:

- what one Australian-equities `M5` bar represents;
...
- the `AUSTRALIAN_EQUITIES_M5_VALIDATOR_V1` validation contract;
...
One canonical Australian-equities `M5` bar represents a half-open five-minute interval within `ASX_NORMAL_TRADING_V1`.

Intervals begin at 10:00 Australia/Sydney and continue to the approved normal-trading close.

## 5.3 Bar Meaning

A complete Australian-equities `M5` bar contains one approved ASX normal-trading source scope's OHLC and compatible volume for one registered security and adjustment basis.

A bar MAY originate through `DIRECT_PROVIDER_M5` or `DIRECT_OPERATOR_M5`.

M5 is the lowest authorised Australian-equities timeframe in Version 1.
...
Intervals advance in exact five-minute increments until the approved normal-trading close.
...
```text
first bar = 10:00–10:05
final bar = 15:55–16:00
```

All 72 ordinary intervals are full five-minute intervals.
...
```text
final bar = 14:05–14:10
```

All 50 intervals are full five-minute intervals.
...
|---|---|---|---|---|
| Twelve Data `5min` | Provider intraday label proven to represent interval open | Request `Australia/Sydney` | Validate against the approved 10:00-based normal-trading grid, then convert to UTC | Symbol, exchange mapping, phase scope, adjustment basis, and effective range MUST validate |
...
Every canonical `M5` bar is owned by the `Australia/Sydney` civil trading date of its approved market-service profile.
...
- operating MIC `XASX` or another separately approved MIC;
...
Fragarach MUST NOT construct M5 bars from ticks, quotes, snapshots, larger intervals, another venue, a CFD, an index, or a linked underlying instrument without separate constitutional authority.
...
The ordinary 15:55–16:00 interval is complete at 16:00.

On an approved 14:10 early close, the 14:05–14:10 interval is complete at 14:10.
...
end_date   = E + 5 minutes represented in Australia/Sydney
```
...
```text
144 expected M5 intervals, equal to two normal regular sessions
```
...
Every `M5` evidence lane MUST materialise an exact effective range for the registered security, security form, ratio where applicable, provider symbol, market operator, operating MIC, market-service profile, source scope, phase and trade-class scope, adjustment basis, and provider interval.
...
The expected `M5` set consists of the intervals generated from the approved market-operator-and-service regular-session schedule within the lane effective range.
...
```text
AUSTRALIAN_EQUITIES_M5_VALIDATOR_V1
```
...
- market code `EQUITIES_AU` and timeframe `M5`;
- market operator, operating MIC, trading platform, and market-service profile;
...
- construction method;
- effective range;
- validator `AUSTRALIAN_EQUITIES_M5_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant of the latest accepted closed `M5` bar, together with its source, market-service profile, phase scope, and validation state.
...
Approval of `AUSTRALIAN_EQUITIES_M5_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for Australian Equities `M5` evidence under the approved market-service profiles defined herein.
