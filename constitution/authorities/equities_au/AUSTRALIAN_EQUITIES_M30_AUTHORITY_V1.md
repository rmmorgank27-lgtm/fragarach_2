# AUSTRALIAN EQUITIES M30 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `AUSTRALIAN_EQUITIES_M30_AUTHORITY_V1`  
**Market Name:** Australian Equities  
**Market Code:** `EQUITIES_AU`  
**Timeframe:** `M30`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/equities_au/AUSTRALIAN_EQUITIES_M30_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `M30` evidence within the Australian Equities market ecosystem of Fragarach II.

It establishes:

- what one Australian-equities `M30` bar represents;
...
- the `AUSTRALIAN_EQUITIES_M30_VALIDATOR_V1` validation contract;
...
**Timeframe Code:** `M30`  
**Nominal Duration:** Thirty minutes, with an approved session-end shortened interval  
...
One canonical Australian-equities `M30` bar represents a half-open interval within `ASX_NORMAL_TRADING_V1`.

Intervals begin at 10:00 Australia/Sydney and continue to the approved normal-trading close.

## 5.3 Bar Meaning

A complete Australian-equities `M30` bar contains one approved ASX normal-trading source scope's OHLC and compatible volume for one registered security and adjustment basis.
...
This comprises eight full 30-minute bars and one complete 10-minute session-end bar.
...
Intervals advance in exact 30-minute increments until the approved normal-trading close.

## 6.2 Boundary Rule

Every interval uses `[start, end)`.

Auction, post-close, crossing, Adjust, and other phase assignments are outside this normal-trading-only profile.
...
```text
first bar = 10:00–10:30
final bar = 15:30–16:00
```

All 12 ordinary intervals are full 30-minute intervals.

## 6.5 Early-Close Example

For an approved 14:10 close:

```text
final full bar = 13:30–14:00
session-end bar = 14:00–14:10
```
...
|---|---|---|---|---|
| Twelve Data `30min` | Provider intraday label proven to represent interval open | Request `Australia/Sydney` | Validate against the approved 10:00-based normal-trading grid, then convert to UTC | Symbol, exchange mapping, phase scope, adjustment basis, and effective range MUST validate |
...
Every canonical `M30` bar is owned by the `Australia/Sydney` civil trading date of its approved market-service profile.
...
- `open` is the first eligible price in the governed `M30` interval;
...
A complete ordinary M30 interval requires:
...
Every expected M5 child MUST be present. Explained no-trade intervals do not authorise fabricated child prices.
...
An M30 bar is closed only when:
...
On an approved 14:10 early close, the 14:00–14:10 interval is complete at 14:10.

## 11.3 Latest Closed Bar

The latest closed M30 bar is the greatest canonical interval open whose governed interval has closed and validated.
...
end_date   = E + 30 minutes represented in Australia/Sydney
```
...
```text
24 expected M30 intervals, equal to two normal regular sessions
```
...
Every `M30` evidence lane MUST materialise an exact effective range for the registered security, security form, ratio where applicable, provider symbol, market operator, operating MIC, market-service profile, source scope, phase and trade-class scope, adjustment basis, and provider interval.
...
The expected `M30` set consists of the intervals generated from the approved market-operator-and-service regular-session schedule within the lane effective range.
...
```text
AUSTRALIAN_EQUITIES_M30_VALIDATOR_V1
```
...
- market code `EQUITIES_AU` and timeframe `M30`;
- market operator, operating MIC, trading platform, and market-service profile;
...
- construction method;
- effective range;
- validator `AUSTRALIAN_EQUITIES_M30_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant of the latest accepted closed `M30` bar, together with its source, market-service profile, phase scope, and validation state.
...
Approval of `AUSTRALIAN_EQUITIES_M30_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for Australian Equities `M30` evidence under the approved market-service profiles defined herein.
...
- Twelve Data API documentation: `/time_series`, interval `30min`, bounded date requests, ascending order, timezone selection, `/earliest_timestamp`, and documented maximum response size of 5,000 records.
