# AUSTRALIAN EQUITIES H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `AUSTRALIAN_EQUITIES_H1_AUTHORITY_V1`  
**Market Name:** Australian Equities  
**Market Code:** `EQUITIES_AU`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/equities_au/AUSTRALIAN_EQUITIES_H1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `H1` evidence within the Australian Equities market ecosystem of Fragarach II.

It establishes:

- what one Australian-equities `H1` bar represents;
...
- the `AUSTRALIAN_EQUITIES_H1_VALIDATOR_V1` validation contract;
...
- direct `H1` evidence and only the derived methods expressly permitted in Section 10.
...
**Timeframe Code:** `H1`  
**Nominal Duration:** One nominal hour, with an approved session-end shortened interval  
**Duration in Minutes:** `60 nominal`  
**Time Unit:** `HOUR`  
**Interval Type:** `HYBRID_SESSION_ALIGNED`

## 5.2 Approved Definition

One canonical Australian-equities `H1` bar represents a half-open interval within `ASX_NORMAL_TRADING_V1`.

Intervals begin at 10:00 Australia/Sydney and advance in exact 60-minute increments. The final interval closes at the approved ASX normal-trading close and may be shorter on an official early-close session.

## 5.3 Bar Meaning

A complete Australian-equities `H1` bar contains one approved ASX normal-trading source scope's OHLC and compatible volume for one registered security and adjustment basis.

A bar MAY originate through `DIRECT_PROVIDER_H1`, `DIRECT_OPERATOR_H1`, `DERIVED_FROM_M30`, or `DERIVED_FROM_M5`.

## 5.4 Expected Counts

Expected intervals are calculated from the approved market-service profile:

```text
expected_count = ceiling((normal-trading close - 10:00) / 60 minutes)
```

For the ordinary 10:00–16:00 session:

```text
Expected H1 bars = 6
```

For an approved 10:00–14:10 early-close session:

```text
Expected H1 bars = 5
```

This comprises four full 60-minute bars and one complete 10-minute session-end bar.
...
For `ASX_NORMAL_TRADING_V1`:

```text
10:00:00 Australia/Sydney
```

Intervals advance in exact 60-minute increments until the approved normal-trading close.

## 6.2 Boundary Rule

Every interval uses `[start, end)`.

Opening-auction, randomised-transition, closing-auction, post-close, crossing, and Adjust assignments are outside this normal-trading-only profile.

## 6.3 Alignment Formula

For normal-trading open `O`, close `C`, and interval index `n`:

```text
interval_open(n) = O + n × 60 minutes
interval_end(n)  = min(O + (n + 1) × 60 minutes, C)
```

An interval exists only when `interval_open(n) < C`.
...
## 6.5 Early-Close Example

For an approved 14:10 normal-trading close:
...
The final 10-minute interval is complete at 14:10.

## 6.6 Daylight-Saving Treatment

Every boundary MUST be resolved under historical IANA `Australia/Sydney` rules and converted to UTC.

## 6.7 Excluded Phases and Events

Opening transition, auctions, post-close, crossings, Adjust activity, overnight reports, another venue, and mixed-phase evidence are outside this profile.

A no-trade interval may be classified as explained only with authoritative halt, suspension, deferred-opening, or market-event evidence.
...
The canonical timestamp is the exact UTC instant at which the approved ASX normal-trading interval opens.

## 7.2 Required Companion Ownership

Every canonical row MUST also resolve:

- `session_date` in `Australia/Sydney`;
- `interval_end` in UTC;
...
|---|---|---|---|---|
| Twelve Data `1h` | Provider intraday label proven to represent interval open | Request `Australia/Sydney` | Validate against the approved 10:00-based normal-trading grid, then convert to UTC | Symbol, exchange mapping, phase scope, adjustment basis, and effective range MUST validate |
| Operator-supplied direct `H1` file | Declared interval open or unambiguous equivalent | MUST be declared | Convert to UTC and calculate session owner | Ambiguous labels are rejected |
| Existing accepted immutable `H1` evidence | Recorded accepted meaning | As recorded | Preserve canonical open and provenance | No silent reinterpretation |
...
A direct provider row is eligible only when its labels align to the approved 10:00-based ASX normal-trading grid.

Rows aligned to the randomised opening transition, auctions, post-close, another venue, civil clock hours unrelated to the session profile, or an unresolved aggregate MUST NOT be forced onto the canonical grid.
...
Implementation MUST NOT guess when timezone, open-versus-close meaning, market operator, service profile, phase scope, or trade class is unresolved.
...
Every canonical `H1` bar is owned by the `Australia/Sydney` civil trading date of its approved market-service profile.
...
- `open` is the first eligible price in the governed `H1` interval;
...
An H1 bar MAY be accepted through:

1. `DIRECT_PROVIDER_H1`;
2. `DIRECT_OPERATOR_H1`;
3. `DERIVED_FROM_M30`;
4. `DERIVED_FROM_M5`.

## 10.2 Derived H1

A full 60-minute H1 interval requires:

```text
2 complete M30 bars, or
12 complete M5 bars
```

The 14:00–14:10 early-close interval requires:

```text
1 complete 10-minute M30 session-end interval, or
2 complete M5 bars
```

## 10.3 Contributor Identity

All contributors MUST share security identity, security form, ratio where applicable, identity segment, provider, operating MIC `XASX`, market-service profile, normal-trading-only scope, source scope, currency, price basis, adjustment basis, corporate-action segment, and effective range.
...
Every expected child interval MUST be present. Halt, suspension, or market-event evidence may explain absence but does not authorise fabrication.

## 10.6 Adjustment Restriction

Rollup MUST preserve contributor adjustment basis.

## 10.7 Forbidden Construction

Cross-provider, cross-market-operator, cross-MIC, cross-service-profile, cross-phase, cross-trade-class, cross-source-scope, cross-adjustment, and alternative-session construction are prohibited.
...
An H1 bar is closed only when:

1. its canonical end is at or before the approved normal-trading close;
...
The ordinary 15:00–16:00 interval is complete at 16:00.

On an approved 14:10 early close, the 14:00–14:10 interval is complete at 14:10. It MUST NOT remain open until 15:00.

## 11.3 Latest Closed Bar

The latest closed H1 bar is the greatest canonical interval open whose complete governed interval has closed and validated.
...
end_date   = E + 60 minutes represented in Australia/Sydney
```
...
```text
500 full regular trading sessions
```
...
```text
12 expected H1 intervals, equal to two normal regular sessions
```
...
Every `H1` evidence lane MUST materialise an exact effective range for the registered security, security form, ratio where applicable, provider symbol, market operator, operating MIC, market-service profile, source scope, phase and trade-class scope, adjustment basis, and provider interval.
...
- provider `H1` availability start;
...
The expected `H1` set consists of the intervals generated from the approved market-operator-and-service regular-session schedule within the lane effective range.
...
```text
AUSTRALIAN_EQUITIES_H1_VALIDATOR_V1
```
...
- market code `EQUITIES_AU` and timeframe `H1`;
- market operator, operating MIC, trading platform, and market-service profile;
...
An active `H1` lane MUST bind at least:
...
- construction method;
- effective range;
- validator `AUSTRALIAN_EQUITIES_H1_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant of the latest accepted closed `H1` bar, together with its source, market-service profile, phase scope, and validation state.
...
Approval of `AUSTRALIAN_EQUITIES_H1_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for Australian Equities `H1` evidence under the approved market-service profiles defined herein.
...
- Twelve Data API documentation: `/time_series`, interval `1h`, bounded date requests, ascending order, timezone selection, `/earliest_timestamp`, and documented maximum response size of 5,000 records.
