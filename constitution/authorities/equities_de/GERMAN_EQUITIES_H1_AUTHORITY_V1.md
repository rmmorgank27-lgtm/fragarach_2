# GERMAN EQUITIES H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `GERMAN_EQUITIES_H1_AUTHORITY_V1`  
**Market Name:** German Equities  
**Market Code:** `EQUITIES_DE`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/equities_de/GERMAN_EQUITIES_H1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `H1` evidence within the German Equities market ecosystem of Fragarach II.

It establishes:

- what one German-equities `H1` bar represents;
...
- the `GERMAN_EQUITIES_H1_VALIDATOR_V1` validation contract;
...
GERMAN_EQUITIES_H1_AUTHORITY_V1
...
One canonical German-equities `H1` bar represents a half-open interval within `XETRA_REGULAR_CONTINUOUS_V1`:

```text
[canonical interval open, canonical interval end)
```

Intervals begin at 09:00 Europe/Berlin and advance in exact 60-minute increments. The final interval closes at 17:30 and is therefore 30 minutes long on an ordinary session.

The final shortened interval is complete, not provisional, when its governed session-end boundary has closed.

## 5.3 Bar Meaning

A complete German-equities `H1` bar contains one approved Xetra continuous-source scope's OHLC and compatible volume for one registered security and adjustment basis.

A bar MAY originate through `DIRECT_PROVIDER_H1`, `DIRECT_OPERATOR_H1`, `DERIVED_FROM_M30`, or `DERIVED_FROM_M5`.

## 5.4 Expected Counts

Expected intervals are calculated from the approved venue-session profile:

```text
expected_count = ceiling((continuous close - continuous open) / 60 minutes)
```
...
This comprises eight full 60-minute bars and one complete 30-minute session-end bar.

No universal count overrides an effective-dated official schedule.
...
Intervals advance in exact 60-minute increments until 17:30.
...
An observation exactly at `end` belongs to the next interval. Auction and Trade-at-Close assignments are outside this continuous-only profile.

## 6.3 Alignment Formula

For continuous open `O`, continuous close `C`, and interval index `n`:
...
## 6.5 Auction Transition

The opening auction may transition to continuous trading after 09:00 for a particular segment or instrument.

The 09:00–10:00 interval remains canonical. Missing trades before the official transition may be explained by phase evidence but MUST NOT be filled.

## 6.6 Exceptional Schedules

The same formula applies to an approved exceptional close. The final interval ends exactly at the approved continuous close and MAY be shorter than 60 minutes.
...
## 6.8 Excluded Services and Events
...
A no-trade interval may be classified as explained only with authoritative halt, interruption, or phase-transition evidence.
...
|---|---|---|---|---|
| Twelve Data `1h` | Provider intraday label proven to represent interval open | Request `Europe/Berlin` | Validate against the approved venue-session grid, then convert to UTC | Symbol, MIC/trading-system scope, phase scope, adjustment basis, and effective range MUST validate |
...
Every canonical `H1` bar is owned by the `Europe/Berlin` civil trading date of its approved venue and venue-session profile.
...
The ordinary final 17:00–17:30 interval requires:

```text
1 complete M30 bar, or
6 complete M5 bars
```

An exceptional session-end interval requires the exact approved child set implied by its boundary.

## 10.3 Contributor Identity

All contributors MUST share security identity, identity segment, provider, MIC `XETR`, trading system, venue-session profile, continuous-only phase scope, source scope, currency, price basis, adjustment basis, corporate-action segment, and effective range.
...
Every expected child interval MUST be present. Halt, interruption, or auction-transition evidence may explain absence but does not authorise fabrication.
...
The ordinary 17:00–17:30 interval is complete at 17:30. It MUST NOT remain open until 18:00.
...
end_date   = E + 60 minutes represented in Europe/Berlin
```
...
Every `H1` evidence lane MUST materialise an exact effective range for the registered security, provider symbol, registered venue and trading system, source scope, trading-system scope, venue-session profile, session scope, adjustment basis, and provider interval.
...
The expected `H1` set consists of the intervals generated from the approved venue-and-trading-system regular-session schedule within the lane effective range.
...
```text
GERMAN_EQUITIES_H1_VALIDATOR_V1
```
...
- market code `EQUITIES_DE` and timeframe `H1`;
- venue name, MIC, trading system, and venue-session profile;
...
- construction method;
- effective range;
- validator `GERMAN_EQUITIES_H1_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant of the latest accepted closed `H1` bar, together with its source, venue-session profile, phase scope, and validation state.
...
Approval of `GERMAN_EQUITIES_H1_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for German Equities `H1` evidence under the approved venue-session profiles defined herein.
