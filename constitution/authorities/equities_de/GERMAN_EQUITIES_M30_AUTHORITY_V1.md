# GERMAN EQUITIES M30 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `GERMAN_EQUITIES_M30_AUTHORITY_V1`  
**Market Name:** German Equities  
**Market Code:** `EQUITIES_DE`  
**Timeframe:** `M30`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/equities_de/GERMAN_EQUITIES_M30_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `M30` evidence within the German Equities market ecosystem of Fragarach II.

It establishes:

- what one German-equities `M30` bar represents;
...
- the `GERMAN_EQUITIES_M30_VALIDATOR_V1` validation contract;
...
GERMAN_EQUITIES_M30_AUTHORITY_V1
...
One canonical German-equities `M30` bar represents a half-open 30-minute interval within `XETRA_REGULAR_CONTINUOUS_V1`.

Intervals begin at 09:00 Europe/Berlin and continue to the 17:30 continuous close.

## 5.3 Bar Meaning

A complete German-equities `M30` bar contains one approved Xetra continuous-source scope's OHLC and compatible volume for one registered security and adjustment basis.
...
Intervals advance in exact 30-minute increments until 17:30.
...
```text
first bar = 09:00–09:30
final bar = 17:00–17:30
```

All 17 ordinary intervals are full 30-minute intervals.

## 6.5 Auction Transition and No-Trade Periods

The opening auction may delay the first continuous trade beyond 09:00. The first canonical interval remains 09:00–09:30.
...
|---|---|---|---|---|
| Twelve Data `30min` | Provider intraday label proven to represent interval open | Request `Europe/Berlin` | Validate against the approved venue-session grid, then convert to UTC | Symbol, MIC/trading-system scope, phase scope, adjustment basis, and effective range MUST validate |
...
Every canonical `M30` bar is owned by the `Europe/Berlin` civil trading date of its approved venue and venue-session profile.
...
An exceptional shortened interval requires the exact approved M5 child set implied by its boundary.
...
The ordinary 17:00–17:30 interval is complete at 17:30.
...
end_date   = E + 30 minutes represented in Europe/Berlin
```
...
```text
34 expected M30 intervals, equal to two normal regular sessions
```
...
Every `M30` evidence lane MUST materialise an exact effective range for the registered security, provider symbol, registered venue and trading system, source scope, trading-system scope, venue-session profile, session scope, adjustment basis, and provider interval.
...
The expected `M30` set consists of the intervals generated from the approved venue-and-trading-system regular-session schedule within the lane effective range.
...
```text
GERMAN_EQUITIES_M30_VALIDATOR_V1
```
...
- market code `EQUITIES_DE` and timeframe `M30`;
- venue name, MIC, trading system, and venue-session profile;
...
- construction method;
- effective range;
- validator `GERMAN_EQUITIES_M30_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant of the latest accepted closed `M30` bar, together with its source, venue-session profile, phase scope, and validation state.
...
Approval of `GERMAN_EQUITIES_M30_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for German Equities `M30` evidence under the approved venue-session profiles defined herein.
