...
**Authority Name:** `FX_M5_AUTHORITY_V1`  
**Market Name:** Foreign Exchange  
**Market Code:** `FX`  
**Timeframe:** `M5`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/fx/FX_M5_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `M5` evidence within the Foreign Exchange market ecosystem of Fragarach II.

It establishes:

- what one FX `M5` bar represents;
...
- the `FX_M5_VALIDATOR_V1` validation contract;
...
One canonical FX `M5` bar represents the half-open interval:
...
A complete FX `M5` bar contains one approved source's declared price-basis OHLC for exactly five minutes.
...
```text
Expected bars per FX trading day = 288
Expected bars per FX week        = 1440
```

The weekend closure contains no expected `M5` intervals.
...
A local interval open is aligned only when it has a minute divisible by `5`, second `00`, and fractional second `0` and lies within the canonical FX operational week.
...
- A provider row inside the weekend closure is not canonical FX `M5` evidence unless a named exception exists.
...
A date without a time cannot identify an FX `M5` interval and MUST NOT enter an active `M5` lane.
...
FX `M5` intervals routinely cross New York civil midnight as a sequence, but each individual fixed-duration interval retains one owner date.
...
FX `M5` evidence is accepted only as a direct provider bar or a direct operator-supplied M5 bar under this Version 1 authority.
...
Twelve Data `5min` bars are approved as direct FX `M5` evidence only when:
...
There is no universal FX `M5` start date.
...
**Validator Authority:** `FX_M5_VALIDATOR_V1`
...
An FX `M5` lane may become `ACTIVE` only when:

- the instrument is registered as FX;
- `FX_BASE_DOCTRINE_V1` is approved;
- `FX_M5_AUTHORITY_V1` is approved;
...
- `FX_M5_VALIDATOR_V1` is implemented without semantic change.
...
- approval of `FX_M5_AUTHORITY_V1`;
...
- align FX `M5` bars to an unrelated UTC epoch grid without New York-session validation;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial FX `M5` constitutional authority drafted | Establish approved `M5` operational truth before implementation | PENDING |
...
- `FX_M5_VALIDATOR_V1` is accepted;
...
> `FX_M5_AUTHORITY_V1` is the approved constitutional authority for `M5` evidence lanes within the Foreign Exchange market ecosystem of Fragarach II. All specifications, implementations, acquisitions, validations, constructions, migrations, lane operations, and acceptance proofs for FX `M5` MUST conform to it.
...
> An FX `M5` bar is not merely a provider row labelled `5min`.  
