...
**Authority Name:** `FX_H1_AUTHORITY_V1`  
**Market Name:** Foreign Exchange  
**Market Code:** `FX`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/fx/FX_H1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `H1` evidence within the Foreign Exchange market ecosystem of Fragarach II.

It establishes:

- what one FX `H1` bar represents;
...
- the `FX_H1_VALIDATOR_V1` validation contract;
...
One canonical FX `H1` bar represents the half-open interval:
...
A complete FX `H1` bar contains one approved source's declared price-basis OHLC for exactly one hour.
...
```text
Expected bars per FX trading day = 24
Expected bars per FX week        = 120
```

The weekend closure contains no expected `H1` intervals.
...
A local interval open is aligned only when it has minute `00`, second `00`, and fractional second `0` and lies within the canonical FX operational week.
...
- A provider row inside the weekend closure is not canonical FX `H1` evidence unless a named exception exists.
...
A date without a time cannot identify an FX `H1` interval and MUST NOT enter an active `H1` lane.
...
FX `H1` intervals routinely cross New York civil midnight as a sequence, but each individual fixed-duration interval retains one owner date.
...
FX `H1` evidence may be accepted from an approved direct `H1` source or constructed from the approved lower timeframes listed below.
...
An H1 bar may be derived from exactly 2 canonical M30 bars or exactly 12 canonical M5 bars. The contributing sequence MUST begin at the canonical H1 open and end at the canonical H1 close without gaps, overlaps, provider changes, or price-basis changes.
...
|---|---|---|---|
| `M30` | YES, CONDITIONAL | `FX_M30_AUTHORITY_V1` approved; exactly 2 eligible M30 bars; same instrument, provider, price basis, and session owner | Preferred derived source where complete |
| `M5` | YES, CONDITIONAL | `FX_M5_AUTHORITY_V1` approved; exactly 12 eligible M5 bars; same instrument, provider, price basis, and session owner | Fine-grained derived source |
...
Twelve Data `1h` bars are approved as direct FX `H1` evidence only when:
...
There is no universal FX `H1` start date.
...
**Validator Authority:** `FX_H1_VALIDATOR_V1`
...
An FX `H1` lane may become `ACTIVE` only when:

- the instrument is registered as FX;
- `FX_BASE_DOCTRINE_V1` is approved;
- `FX_H1_AUTHORITY_V1` is approved;
...
- `FX_H1_VALIDATOR_V1` is implemented without semantic change.
...
- approval of `FX_H1_AUTHORITY_V1`;
...
- align FX `H1` bars to an unrelated UTC epoch grid without New York-session validation;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial FX `H1` constitutional authority drafted | Establish approved `H1` operational truth before implementation | PENDING |
...
- `FX_H1_VALIDATOR_V1` is accepted;
...
> `FX_H1_AUTHORITY_V1` is the approved constitutional authority for `H1` evidence lanes within the Foreign Exchange market ecosystem of Fragarach II. All specifications, implementations, acquisitions, validations, constructions, migrations, lane operations, and acceptance proofs for FX `H1` MUST conform to it.
...
> An FX `H1` bar is not merely a provider row labelled `1h`.  
