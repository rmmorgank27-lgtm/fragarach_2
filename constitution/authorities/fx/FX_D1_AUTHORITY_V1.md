...
**Authority Name:** `FX_D1_AUTHORITY_V1`  
**Market Name:** Foreign Exchange  
**Market Code:** `FX`  
**Timeframe:** `D1`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/fx/FX_D1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `D1` evidence within the Foreign Exchange market ecosystem of Fragarach II.

It establishes:

- what one FX `D1` bar represents;
...
- the `FX_D1_VALIDATOR_V1` validation contract;
...
- FX market identity;
- base and quote currency semantics;
...
**Timeframe Code:** `D1`  
**Nominal Duration:** One canonical FX trading day  
**Time Unit:** `DAY`  
**Interval Type:** `SESSION-DERIVED`

## 5.2 Approved Definition

One canonical FX `D1` bar represents one owned FX trading day under `FX_24X5_NEW_YORK_ROLLOVER_V1`.
...
Therefore, an observation occurring exactly at the 17:00 boundary belongs to the next FX trading day.

## 5.3 Bar Meaning

A complete FX `D1` bar contains the approved provider's declared price-basis OHLC for the owned trading day.
...
There is no separate extended-hours category for spot FX under this authority.

## 5.4 Actual Elapsed Duration

`D1` is not defined as a fixed number of UTC seconds.

Because the interval is anchored to `America/New_York`, an interval crossing a daylight-saving transition MAY contain 23, 24, or 25 elapsed hours while remaining one valid FX trading day.
...
- MUST NOT combine two owned FX trading days;
...
A shortened or incomplete evidence interval caused by provider outage, acquisition interruption, instrument launch, suspension, or incomplete source coverage MUST NOT be silently promoted to a complete derived `D1` bar.

Such evidence MAY be retained as `PARTIAL` or `PROVISIONAL` with visible provenance.

There is no generic FX early-close rule.

Any exceptional market-wide closure or shortened trading day requires a documented exception under Section 21.
...
2. the source is approved for FX `D1`;
3. provenance declares the date to be the provider's daily label or the FX close-date owner;
4. the date resolves to an expected Monday-through-Friday FX trading date or a documented exception;
...
The canonical FX trading day is owned by the New York civil date on which the session closes at 17:00.

## 8.2 Timeframe-Specific Ownership

Exactly one canonical `D1` owner date exists for each expected Monday-through-Friday FX trading day.
...
The FX `D1` session crosses New York midnight by design.
...
- The Friday-owned bar is the final normal bar of the FX week.
- The Monday-owned bar is the first normal bar of the next FX week.
- A bar belongs to the calendar month of its owner date, even when it opened in the preceding month.
- A public holiday does not automatically remove an expected owner date.
...
Each bar MUST retain one declared price basis, including:
...
Unlike price bases MUST NOT be silently combined or treated as exact substitutes.
...
FX `D1` evidence MAY be accepted from:
...
|---|---|---|---|
| `H1` | YES, CONDITIONAL | `FX_H1_AUTHORITY_V1` approved; same instrument; compatible provider and price basis; complete interval coverage | May construct `D1` after H1 authority approval |
| `M30` | YES, CONDITIONAL | `FX_M30_AUTHORITY_V1` approved; same instrument; compatible provider and price basis; complete interval coverage | May construct `D1` after M30 authority approval |
| `M5` | YES, CONDITIONAL | `FX_M5_AUTHORITY_V1` approved; same instrument; compatible provider and price basis; complete interval coverage | Preferred fine-grained rollup source when complete |
...
Twelve Data `1day` bars are approved as direct FX `D1` evidence when:

- the registered provider symbol resolves;
- the request uses the contract in Section 12;
- the response date maps under Section 7;
- the price basis is declared;
- the row validates under `FX_D1_VALIDATOR_V1`;
...
A logical FX `D1` interval is closed when its canonical interval end has passed:
...
- its OHLC validates;
- its source and price basis are declared;
...
|---|---|
| `symbol` | Registered Twelve Data provider symbol for the FX instrument |
| `interval` | `1day` |
...
There is no universal start date for all FX `D1` lanes.

For each instrument and evidence source, the effective start is the latest of:

1. the instrument start approved under `FX_BASE_DOCTRINE_V1`;
2. the registered currency-regime start;
...
|---|---|---|---|
| Twelve Data | Result of `/earliest_timestamp` for the registered symbol and `1day`, captured with provenance and confirmed by successful acquisition | Latest available bar, subject to latest-closed filtering | Provider coverage does not override instrument-regime authority |
...
- material quote convention change.
...
A holiday in one country is not sufficient to declare an FX date `NOT_EXPECTED`.
...
- a gap that leaves an entire FX trading week with no valid `D1` evidence is `HIGH` historical materiality;
...
This authority inherits Section 10 of `FX_BASE_DOCTRINE_V1`.

An FX `D1` bar represents one provider's declared price basis, not a universal consolidated market price.
...
**Validator Authority:** `FX_D1_VALIDATOR_V1`
...
- negative FX prices are invalid unless a named constitutional exception exists;
- zero FX prices are invalid unless a named constitutional exception exists;
...
An FX `D1` lane may become `ACTIVE` only when:

- the instrument is registered as FX;
- `FX_BASE_DOCTRINE_V1` is approved;
- `FX_D1_AUTHORITY_V1` is approved;
...
- `FX_D1_VALIDATOR_V1` is implemented without semantic change.
...
Freshness is measured against the latest expected closed FX `D1` owner date determined by Section 11.2.
...
The operator-facing Current-As-Of Truth Date for an FX `D1` lane is the owner date of the latest accepted complete selected bar.
...
# 20. Provider Precedence for FX D1
...
- `FX_BASE_DOCTRINE_V1` is approved;
- `FX_D1_AUTHORITY_V1` is approved;
- the instrument is registered under `FX`;
...
- align FX `D1` to UTC midnight as a physical market interval;
...
- assume every public holiday is a global FX closure;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial FX `D1` constitutional authority drafted | Establish approved D1 operational truth before further timeframe implementation | PENDING |
...
- `FX_D1_VALIDATOR_V1` is accepted;
...
> `FX_D1_AUTHORITY_V1` is the approved constitutional authority for `D1` evidence lanes within the Foreign Exchange market ecosystem of Fragarach II. All specifications, implementations, acquisitions, validations, constructions, migrations, lane operations, and acceptance proofs for FX `D1` MUST conform to it.
...
> An FX daily bar is not merely a provider row labelled `1day`.  
