# METALS H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `METALS_H1_AUTHORITY_V1`  
**Market Name:** Spot Precious Metals  
**Market Code:** `METALS`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/metals/METALS_H1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `H1` evidence within the Spot Precious Metals market ecosystem of Fragarach II.

It establishes:

- what one metals `H1` bar represents;
...
- the `METALS_H1_VALIDATOR_V1` validation contract;
...
- timeframe code `H1`;
- intervals within `METALS_24X5_NEW_YORK_ROLLOVER_V1`;
- evidence whose provider, source scope, price basis, pricing unit, timestamp meaning, and effective range are known;
- direct `H1` provider or operator-supplied evidence;
- derived `H1` evidence only where Section 10 explicitly permits it.
...
- metals market identity, base-metal identity, quote-asset semantics, and troy-ounce unit authority;
...
**Timeframe Code:** `H1`  
**Nominal Duration:** One hour  
**Duration in Minutes:** `60`  
**Time Unit:** `HOUR`  
**Interval Type:** `HYBRID`

The bar duration is fixed, but valid intervals exist only inside the session-derived metals operational week and are owned under the 17:00 New York rollover model.

## 5.2 Approved Definition

One canonical metals `H1` bar represents the half-open interval:

```text
[canonical interval open, canonical interval open + 60 minutes)
```

The interval open MUST satisfy:

```text
local minute = 00 and local second = 00
```

in `America/New_York`, and the interval MUST lie wholly inside an owned metals trading day.

## 5.3 Bar Meaning

A complete metals `H1` bar contains one approved source's declared price-basis OHLC for exactly one hour.

A bar MAY originate through `DIRECT_PROVIDER_H1`, `DIRECT_OPERATOR_H1`, `DERIVED_FROM_M30`, or `DERIVED_FROM_M5`.

The construction method, provider, provider symbol, source scope, price basis, pricing unit, source timestamp, canonical timestamp, session owner, acquisition run, and effective segment MUST remain explicit in provenance.

There is no separate extended-hours category for spot precious metals.
...
```text
Expected bars per metals trading day = 24
Expected bars per metals week        = 120
```

The canonical weekend closure contains no expected `H1` intervals.
...
The alignment origin is each metals trading-day boundary at:

```text
17:00:00 America/New_York
```

Intervals advance in exact 60-minute increments from that boundary until the next 17:00 boundary.
...
An interval beginning exactly at 17:00 belongs to the next owned metals trading day.

## 6.3 Alignment Formula

A local interval open is aligned only when it has minute `00`, second `00`, and fractional second `0` and lies within the canonical metals operational week.
...
```text
interval_end = interval_open + 60 elapsed minutes
```
...
- the first `H1` bar opens at `17:00` on the preceding New York civil date;
- the final `H1` bar opens at `16:00` on the owned New York civil date;
...
```text
Summer final-bar open: 2026-07-06 16:00 America/New_York = 2026-07-06T20:00:00Z
...
- The final normal weekly interval opens Friday at `16:00` New York and closes Friday at 17:00.
...
- A provider row inside the weekend closure is not canonical metals `H1` evidence unless a named exception exists.
...
|---|---|---|---|---|
| Twelve Data `1h` | Interval-open local datetime under this authority | Request MUST specify `America/New_York` | Interpret as aligned local interval open, convert to exact UTC instant | Symbol, alignment, price basis, response metadata, and effective range MUST validate |
| Operator-supplied direct `H1` file | Declared interval open or unambiguous equivalent | MUST be declared in manifest | Convert declared interval open to UTC and calculate session owner | Ambiguous timestamps are not accepted |
| Existing accepted immutable `H1` evidence | Existing accepted interval-open meaning | As recorded | Preserve canonical UTC open and ownership | Original provenance remains immutable |
...
A date without a time cannot identify a metals `H1` interval and MUST NOT enter an active `H1` lane.
...
A metals trading day is owned by the New York civil date on which the session closes at 17:00.

## 8.2 Timeframe-Specific Ownership

A `H1` interval belongs to owner date `D` only when its complete interval lies within:
...
Metals `H1` intervals routinely cross New York civil midnight as a sequence, but each individual fixed-duration interval retains one owner date.
...
- Public holidays do not automatically remove expected metals intraday intervals.
...
For a direct provider bar, OHLC retains the provider's declared `H1` aggregation and price basis.
...
Direct Twelve Data spot-metals bars under Version 1 SHALL be classified as:
...
The same direct evidence MUST retain:
...
A separately approved provider mapping MAY establish a more specific source scope or authorised unit transformation. No such detail may be inferred from the symbol alone.
...
Metals `H1` evidence may be accepted from an approved direct `H1` source or constructed from the approved lower timeframes listed below.
...
An H1 bar may be derived from exactly 2 canonical M30 bars or exactly 12 canonical M5 bars. The contributing sequence MUST begin at the canonical H1 open and end at the canonical H1 close without gaps, overlaps, provider changes, source-scope changes, price-basis changes, or unit changes.

A derived `H1` bar may be classified `CLOSED` only when all 2 M30 bars or 12 M5 bars are present, canonical, closed, contiguous, and eligible.

## 10.3 Source Timeframe Eligibility

| Source Timeframe | Permitted | Conditions | Notes |
|---|---|---|---|
| `M30` | YES, CONDITIONAL | `METALS_M30_AUTHORITY_V1` approved; exactly 2 eligible M30 bars; same instrument, provider, source scope, price basis, pricing unit, and session owner | Preferred derived source where complete |
| `M5` | YES, CONDITIONAL | `METALS_M5_AUTHORITY_V1` approved; exactly 12 eligible M5 bars; same instrument, provider, source scope, price basis, pricing unit, and session owner | Fine-grained derived source |
| `M1`, `M3`, `M15`, tick | NO under Version 1 | Separate authority required | Retain evidence but do not construct canonical H1 |

## 10.4 Direct Provider Authority

Twelve Data `1h` bars are approved as direct metals `H1` evidence only when:
...
- the source scope is `TWELVE_DATA_PROVIDER_AGGREGATE` or another separately approved value;
- the price basis is `PROVIDER_AGGREGATE` or another separately approved value;
- the pricing unit is `QUOTE_ASSET_PER_TROY_OUNCE` or an explicitly authorised transformation;
- the row lies within the materialised effective segment;
- `METALS_H1_VALIDATOR_V1` passes.
...
If one or more of the required 2 M30 bars or 12 M5 bars are missing, the partial aggregate MAY be retained as `PARTIAL` or `PROVISIONAL`, but it MUST NOT be promoted as a complete canonical `H1` bar.
...
Complete eligible `H1` evidence MAY support approved D1 construction, but only under those higher-timeframe authorities.
...
A canonical `H1` interval is logically closed when:

```text
current instant >= canonical interval end
```

At 16:59:59 New York, the 16:00 H1 interval is still open. At 17:00:00, it is logically closed and the 17:00 interval begins under the next trading-day owner.
...
The latest expected closed `H1` interval is the greatest canonical interval inside the operational week whose interval end is less than or equal to the current instant.
...
| `end_date` | Desired final canonical interval-open local datetime plus 60 minutes |
...
end_date   = E + 60 minutes represented in America/New_York
```
...
```text
maximum 4,000 expected H1 intervals per request
```

The default chunk span MUST NOT exceed:

```text
160 full metals trading days
```
...
```text
48 expected H1 intervals
```

which equals two full metals trading days.

Overlap provides deterministic reassembly and revision evidence.

## 12.4 Incremental Acquisition

An incremental request SHOULD begin at least `48` expected intervals before the latest accepted closed interval and continue through the interval immediately after the latest expected closed open time, followed by canonical filtering.
...
There is no universal metals `H1` start date.
...
3. the earliest timestamp returned by Twelve Data `/earliest_timestamp` for the registered symbol and `1h`;
...
Within an active effective segment, one `H1` bar is expected at every canonical alignment point from Sunday 17:00 through Friday 17:00 New York, excluding only intervals named by an approved exception.

Expected counts are `24` per trading day and `120` per normal week.
...
- an interval inside a recurring provider maintenance or publication break only when that break, source scope, effective range, and lane-specific treatment are explicitly approved.

A provider-specific break does not remove the interval from the market-wide metals calendar and does not excuse missing evidence from unrelated providers.
...
| `PROVIDER_MAINTENANCE` | Expected market interval falls inside an approved provider-specific maintenance or publication break | Treat as lane-specific non-publication; retain market expectedness and continue with other compatible evidence |
...
- a gap among the latest `48` expected closed `H1` intervals is `HIGH` operational priority;
...
1. re-acquire the same interval from the same provider, source scope, price basis, and pricing unit;
2. use existing accepted immutable evidence;
3. accept a compatible operator-supplied direct `H1` file;
...
Rows for the same interval but different provider, source scope, price basis, pricing unit, construction method, or source role are parallel evidence and MUST NOT be silently collapsed as identical authority.
...
No `H1`-specific smoothing, inversion, interpolation, synthetic-cross creation, or adjustment is authorised.

Each bar remains one declared price basis, source scope, and pricing unit from one source.
...
**Validator Authority:** `METALS_H1_VALIDATOR_V1`
...
- minute `00`, second `00`, and fractional second `0`;
...
- zero or negative metals prices are invalid unless a named exception exists;
...
1. its local source open satisfies `local minute = 00 and local second = 00`;
2. its canonical UTC timestamp is the exact conversion of that local open;
3. its end is exactly 60 minutes later;
4. both boundaries lie within one owned metals trading day;
...
For a complete normal owner date, the validator expects `24` aligned intervals.

For a complete normal week, it expects `120` aligned intervals.
...
| Missing provider semantics, source scope, price basis, pricing unit, effective start, or parent authority | `BLOCKED` | Stop only affected path; emit compatibility report |
...
```text
registered metals instrument / H1
```

Every evidence row MUST additionally retain provider/source identity, provider symbol, source scope, price basis, pricing unit, construction method, effective segment, evidence-block identity, and validation status.

## 18.2 Activation Gate

A metals `H1` lane may become `ACTIVE` only when:

- the instrument is registered as METALS;
- `METALS_BASE_DOCTRINE_V1` is approved;
- `METALS_H1_AUTHORITY_V1` is approved;
- provider/source role and symbol mapping are approved;
- source scope is explicit;
- price basis is explicit;
- pricing unit is explicit and compatible with the registered troy-ounce authority;
...
- `METALS_H1_VALIDATOR_V1` is implemented without semantic change.
...
Every accepted bar MUST trace to instrument registration, lane, evidence block, provider/source, source symbol, acquisition run, requested/received range, parser version, source timestamp, canonical mapping, source scope, price basis, pricing unit, validation result, construction method, contributing evidence where derived, and conflict history.
...
Freshness is measured against the latest expected closed `H1` interval from Section 11.3.
...
| `DELAYED` | Latest accepted interval is exactly one expected closed `H1` interval behind | Continue with visible warning |
| `STALE` | Latest accepted interval is two or more expected closed `H1` intervals behind | Continue with prominent warning and repair priority |
...
| 2 | Twelve Data direct `1h` | Primary automated acquisition and uncovered-interval source | Contract, mapping, range, and validation pass |
| 3 | Operator-supplied direct `H1` file | Manual backfill or supplementary source | Manifest, checksum, timezone, timestamp meaning, price basis, and validation required |
| 4 | Complete derived `H1` evidence | Uncovered-interval or verification source | Only where Section 10 permits; one provider and complete contributors |
...
- approval of `METALS_H1_AUTHORITY_V1`;
- registered metals instrument and lane;
- exact 60-minute duration;
...
- `H1` duration or interval meaning;
...
- align metals `H1` bars to an unrelated UTC epoch grid without New York-session validation;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial metals `H1` constitutional authority drafted | Establish approved `H1` operational truth before implementation | PENDING |
...
- `METALS_BASE_DOCTRINE_V1` is approved;
- one hour duration and New York alignment are accepted;
...
- Twelve Data `1h` request and response contract is accepted;
- 4,000-row constitutional ceiling, `160`-day chunk cap, and `48`-interval overlap are accepted;
- exact effective-start materialisation is accepted;
- gap, duplicate, conflict, and repair rules are accepted;
- `METALS_H1_VALIDATOR_V1` is accepted;
...
> `METALS_H1_AUTHORITY_V1` is the approved constitutional authority for `H1` evidence lanes within the Spot Precious Metals market ecosystem of Fragarach II. All specifications, implementations, acquisitions, validations, constructions, migrations, lane operations, and acceptance proofs for metals `H1` MUST conform to it.
...
2. Twelve Data Support — *How to get historical prices*, updated January 12, 2026, documenting `1h` support as an intraday interval, `/earliest_timestamp`, bounded `start_date` and `end_date` requests, omission of `outputsize` for bounded ranges, and a maximum `outputsize` of 5,000:  
...
> A metals `H1` bar is not merely a provider row labelled `1h`.  
> It is an exact 60-minute interval, aligned to New York rollover authority, owned by a trading day, timestamped by its UTC open instant, and proven by immutable evidence.
