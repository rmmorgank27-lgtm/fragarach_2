# METALS M30 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `METALS_M30_AUTHORITY_V1`  
**Market Name:** Spot Precious Metals  
**Market Code:** `METALS`  
**Timeframe:** `M30`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/metals/METALS_M30_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `M30` evidence within the Spot Precious Metals market ecosystem of Fragarach II.

It establishes:

- what one metals `M30` bar represents;
...
- the `METALS_M30_VALIDATOR_V1` validation contract;
...
One canonical metals `M30` bar represents the half-open interval:
...
A complete metals `M30` bar contains one approved source's declared price-basis OHLC for exactly thirty minutes.
...
```text
Expected bars per metals trading day = 48
Expected bars per metals week        = 240
```

The canonical weekend closure contains no expected `M30` intervals.
...
A local interval open is aligned only when it has minute `00` or `30`, second `00`, and fractional second `0` and lies within the canonical metals operational week.
...
- A provider row inside the weekend closure is not canonical metals `M30` evidence unless a named exception exists.
...
A date without a time cannot identify a metals `M30` interval and MUST NOT enter an active `M30` lane.
...
Metals `M30` intervals routinely cross New York civil midnight as a sequence, but each individual fixed-duration interval retains one owner date.
...
Metals `M30` evidence may be accepted from an approved direct `M30` source or constructed from the approved lower timeframes listed below.
...
An M30 bar may be derived from exactly 6 canonical M5 bars. The contributing sequence MUST begin at the canonical M30 open and end at the canonical M30 close without gaps, overlaps, provider changes, source-scope changes, price-basis changes, or unit changes.
...
|---|---|---|---|
| `M5` | YES, CONDITIONAL | `METALS_M5_AUTHORITY_V1` approved; exactly 6 eligible M5 bars; same instrument, provider, source scope, price basis, pricing unit, and session owner | Only approved Version 1 rollup source |
...
Twelve Data `30min` bars are approved as direct metals `M30` evidence only when:
...
- `METALS_M30_VALIDATOR_V1` passes.
...
There is no universal metals `M30` start date.
...
**Validator Authority:** `METALS_M30_VALIDATOR_V1`
...
```text
registered metals instrument / M30
```
...
A metals `M30` lane may become `ACTIVE` only when:

- the instrument is registered as METALS;
- `METALS_BASE_DOCTRINE_V1` is approved;
- `METALS_M30_AUTHORITY_V1` is approved;
...
- `METALS_M30_VALIDATOR_V1` is implemented without semantic change.
...
- approval of `METALS_M30_AUTHORITY_V1`;
...
- align metals `M30` bars to an unrelated UTC epoch grid without New York-session validation;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial metals `M30` constitutional authority drafted | Establish approved `M30` operational truth before implementation | PENDING |
...
- `METALS_M30_VALIDATOR_V1` is accepted;
...
> `METALS_M30_AUTHORITY_V1` is the approved constitutional authority for `M30` evidence lanes within the Spot Precious Metals market ecosystem of Fragarach II. All specifications, implementations, acquisitions, validations, constructions, migrations, lane operations, and acceptance proofs for metals `M30` MUST conform to it.
...
> A metals `M30` bar is not merely a provider row labelled `30min`.  
