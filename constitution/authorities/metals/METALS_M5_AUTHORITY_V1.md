# METALS M5 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `METALS_M5_AUTHORITY_V1`  
**Market Name:** Spot Precious Metals  
**Market Code:** `METALS`  
**Timeframe:** `M5`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/metals/METALS_M5_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `M5` evidence within the Spot Precious Metals market ecosystem of Fragarach II.

It establishes:

- what one metals `M5` bar represents;
...
- the `METALS_M5_VALIDATOR_V1` validation contract;
...
- direct `M5` provider or operator-supplied evidence;
- derived `M5` evidence only where Section 10 explicitly permits it.
...
One canonical metals `M5` bar represents the half-open interval:

```text
[canonical interval open, canonical interval open + 5 minutes)
```

The interval open MUST satisfy:

```text
local minute mod 5 = 0 and local second = 00
```
...
A complete metals `M5` bar contains one approved source's declared price-basis OHLC for exactly five minutes.
...
```text
Expected bars per metals trading day = 288
Expected bars per metals week        = 1440
```

The canonical weekend closure contains no expected `M5` intervals.
...
Intervals advance in exact 5-minute increments from that boundary until the next 17:00 boundary.
...
A local interval open is aligned only when it has a minute divisible by `5`, second `00`, and fractional second `0` and lies within the canonical metals operational week.
...
```text
interval_end = interval_open + 5 elapsed minutes
```
...
- the first `M5` bar opens at `17:00` on the preceding New York civil date;
- the final `M5` bar opens at `16:55` on the owned New York civil date;
...
```text
Summer final-bar open: 2026-07-06 16:55 America/New_York = 2026-07-06T20:55:00Z
...
- The final normal weekly interval opens Friday at `16:55` New York and closes Friday at 17:00.
...
- A provider row inside the weekend closure is not canonical metals `M5` evidence unless a named exception exists.
...
|---|---|---|---|---|
| Twelve Data `5min` | Interval-open local datetime under this authority | Request MUST specify `America/New_York` | Interpret as aligned local interval open, convert to exact UTC instant | Symbol, alignment, price basis, response metadata, and effective range MUST validate |
| Operator-supplied direct `M5` file | Declared interval open or unambiguous equivalent | MUST be declared in manifest | Convert declared interval open to UTC and calculate session owner | Ambiguous timestamps are not accepted |
| Existing accepted immutable `M5` evidence | Existing accepted interval-open meaning | As recorded | Preserve canonical UTC open and ownership | Original provenance remains immutable |
...
A date without a time cannot identify a metals `M5` interval and MUST NOT enter an active `M5` lane.
...
A `M5` interval belongs to owner date `D` only when its complete interval lies within:
...
Metals `M5` intervals routinely cross New York civil midnight as a sequence, but each individual fixed-duration interval retains one owner date.
...
For a direct provider bar, OHLC retains the provider's declared `M5` aggregation and price basis.
...
**Approved Construction Source:** `DIRECT PROVIDER BAR`

Metals `M5` evidence is accepted only as a direct provider bar or a direct operator-supplied M5 bar under this Version 1 authority.
...
Fragarach-side construction from a lower timeframe is not authorised by Version 1. A future M1, M3, or tick authority may amend this rule through a new authority version.

No lower-timeframe rollup is authorised under this version.

## 10.3 Source Timeframe Eligibility

| Source Timeframe | Permitted | Conditions | Notes |
|---|---|---|---|
| `M1` | NO under Version 1 | Separate approved authority required | Evidence may be retained but cannot construct canonical M5 |
| `M3` | NO under Version 1 | Separate approved authority required | Evidence may be retained but cannot construct canonical M5 |
| Tick | NO under Version 1 | Separate approved authority required | Evidence may be retained but cannot construct canonical M5 |
| `M15`, `M30`, `H1`, or higher | NO | Downsampling cannot construct M5 evidence | Not permitted |

## 10.4 Direct Provider Authority

Twelve Data `5min` bars are approved as direct metals `M5` evidence only when:
...
Because no lower-timeframe construction is authorised, missing-source-bar logic does not create a derived M5 candidate. Missing direct M5 intervals remain gaps or source-unavailable states.
...
At 16:59:59 New York, the 16:55 M5 interval is still open. At 17:00:00, it is logically closed and the 17:00 interval begins under the next trading-day owner.
...
The latest expected closed `M5` interval is the greatest canonical interval inside the operational week whose interval end is less than or equal to the current instant.
...
| `end_date` | Desired final canonical interval-open local datetime plus 5 minutes |
...
end_date   = E + 5 minutes represented in America/New_York
```
...
An incremental request SHOULD begin at least `576` expected intervals before the latest accepted closed interval and continue through the interval immediately after the latest expected closed open time, followed by canonical filtering.
...
There is no universal metals `M5` start date.
...
3. the earliest timestamp returned by Twelve Data `/earliest_timestamp` for the registered symbol and `5min`;
...
Within an active effective segment, one `M5` bar is expected at every canonical alignment point from Sunday 17:00 through Friday 17:00 New York, excluding only intervals named by an approved exception.

Expected counts are `288` per trading day and `1440` per normal week.
...
- a gap among the latest `288` expected closed `M5` intervals is `HIGH` operational priority;
...
3. accept a compatible operator-supplied direct `M5` file;
4. use another provider only after separate provider authority approval.
...
No `M5`-specific smoothing, inversion, interpolation, synthetic-cross creation, or adjustment is authorised.
...
**Validator Authority:** `METALS_M5_VALIDATOR_V1`
...
- a minute divisible by `5`, second `00`, and fractional second `0`;
...
1. its local source open satisfies `local minute mod 5 = 0 and local second = 00`;
2. its canonical UTC timestamp is the exact conversion of that local open;
3. its end is exactly 5 minutes later;
...
For a complete normal owner date, the validator expects `288` aligned intervals.

For a complete normal week, it expects `1440` aligned intervals.
...
```text
registered metals instrument / M5
```
...
A metals `M5` lane may become `ACTIVE` only when:

- the instrument is registered as METALS;
- `METALS_BASE_DOCTRINE_V1` is approved;
- `METALS_M5_AUTHORITY_V1` is approved;
...
- `METALS_M5_VALIDATOR_V1` is implemented without semantic change.
...
| `DELAYED` | Latest accepted interval is one, two, or three expected closed `M5` intervals behind | Continue with visible warning |
| `STALE` | Latest accepted interval is four or more expected closed `M5` intervals behind | Continue with prominent warning and repair priority |
...
| 2 | Twelve Data direct `5min` | Primary automated acquisition and uncovered-interval source | Contract, mapping, range, and validation pass |
| 3 | Operator-supplied direct `M5` file | Manual backfill or supplementary source | Manifest, checksum, timezone, timestamp meaning, price basis, and validation required |
| 4 | Complete derived `M5` evidence | Uncovered-interval or verification source | Only where Section 10 permits; one provider and complete contributors |
...
- approval of `METALS_M5_AUTHORITY_V1`;
- registered metals instrument and lane;
- exact 5-minute duration;
...
- `M5` duration or interval meaning;
...
- align metals `M5` bars to an unrelated UTC epoch grid without New York-session validation;
...
|---|---|---|---|---|
| 1.0 | 2026-07-11 | Initial metals `M5` constitutional authority drafted | Establish approved `M5` operational truth before implementation | PENDING |
...
- `METALS_BASE_DOCTRINE_V1` is approved;
- five minutes duration and New York alignment are accepted;
...
- Twelve Data `5min` request and response contract is accepted;
- 4,000-row constitutional ceiling, `13`-day chunk cap, and `576`-interval overlap are accepted;
- exact effective-start materialisation is accepted;
- gap, duplicate, conflict, and repair rules are accepted;
- `METALS_M5_VALIDATOR_V1` is accepted;
...
> `METALS_M5_AUTHORITY_V1` is the approved constitutional authority for `M5` evidence lanes within the Spot Precious Metals market ecosystem of Fragarach II. All specifications, implementations, acquisitions, validations, constructions, migrations, lane operations, and acceptance proofs for metals `M5` MUST conform to it.
...
2. Twelve Data Support — *How to get historical prices*, updated January 12, 2026, documenting `5min` support as an intraday interval, `/earliest_timestamp`, bounded `start_date` and `end_date` requests, omission of `outputsize` for bounded ranges, and a maximum `outputsize` of 5,000:  
...
> A metals `M5` bar is not merely a provider row labelled `5min`.  
> It is an exact 5-minute interval, aligned to New York rollover authority, owned by a trading day, timestamped by its UTC open instant, and proven by immutable evidence.
