# US EQUITIES H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `UK_EQUITIES_H1_AUTHORITY_V1`  
**Market Name:** United Kingdom Equities  
**Market Code:** `EQUITIES_UK`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/equities_uk/UK_EQUITIES_H1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `H1` evidence within the United Kingdom Equities market ecosystem of Fragarach II.

It establishes:

- what one UK-equities `H1` bar represents;
...
- the `UK_EQUITIES_H1_VALIDATOR_V1` validation contract;
...
One canonical UK-equities `H1` bar represents a half-open interval inside the approved primary-venue regular trading session:

```text
[canonical interval open, canonical interval end)
```

Intervals begin at the official regular-session open and advance in exact 60-minute increments. The final interval of a session closes at the official regular-session close even when it is shorter than 60 minutes.
...
A complete UK-equities `H1` bar contains one approved source scope's regular-session OHLC and compatible volume for one registered security and one adjustment basis.

A bar MAY originate through `DIRECT_PROVIDER_H1`, `DIRECT_OPERATOR_H1` or `DERIVED_FROM_M30` or `DERIVED_FROM_M5`.
...
```text
expected_count = ceiling((official session close - official session open) / 60 minutes)
```

For the ordinary 08:00–16:30 regular session:

```text
Expected H1 bars = 9, comprising eight full 60-minute bars and one complete 30-minute session-end bar
```

For an approved 08:00–12:30 shortened session:
...
Intervals advance in exact 60-minute increments until the official session close.
...
For an ordinary 08:00–16:30 session:
- first bar: 08:00–09:00;
- eighth full bar: 15:00–16:00;
- final complete session-end bar: 16:00–16:30.

## 6.5 Official Early Closes

The same formula applies to an official early close. The final interval closes exactly at the approved session close and MAY be shorter than 60 minutes.

## 6.6 Daylight-Saving Treatment

Every interval boundary MUST be resolved under historical IANA `Europe/London` rules and converted to UTC.
...
|---|---|---|---|---|
| Twelve Data `1h` | Provider intraday label proven to represent interval open | Request `Europe/London` | Validate against canonical session grid, then convert to UTC | Symbol, venue scope, session scope, adjustment basis, and effective range MUST validate |
...
A date without time cannot identify a `H1` interval.
...
Every canonical `H1` bar is owned by the approved primary venue's `Europe/London` trading date.
...
- `open` is the first eligible regular-session price in the governed `H1` interval;
...
A full 60-minute H1 interval requires either:
...
The ordinary final 16:00–16:30 session-end H1 interval requires:
...
For an exceptional session-end interval, the required child set is calculated from the approved interval boundary.

## 10.3 Contributor Identity

All contributors MUST share security, provider, venue/consolidated scope, regular-session scope, listing currency, price-display unit, price basis, adjustment basis, corporate-action segment, and effective range.
...
Every expected child interval MUST be present. Authoritative halt evidence may explain a no-trade period but does not authorise fabrication of child prices.

## 10.6 Adjustment Restriction

Rollup MUST preserve the contributor adjustment basis. No adjustment may be created during aggregation.

## 10.7 Forbidden Construction

Cross-provider, cross-venue-scope, cross-trading-service, cross-session, cross-auction-scope, cross-adjustment, and alternative-session construction are prohibited.
...
A shortened final interval ending at the official session close is complete at that close. It MUST NOT remain open until the nominal 60 nominal duration would have elapsed.
...
end_date   = E + 60 minutes represented in Europe/London
```
...
Every `H1` evidence lane MUST materialise an exact effective range for the registered security, provider symbol, primary venue, source scope, session scope, adjustment basis, listing currency, price-display unit, and provider interval.
...
The expected `H1` set consists of the intervals generated from the approved primary-venue regular-session schedule within the lane effective range.
...
- market code `EQUITIES_UK` and timeframe `H1`;
- primary venue, market segment or trading service, and calendar authority;
...
- construction method;
- effective range;
- validator `UK_EQUITIES_H1_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant or session date of the latest accepted closed `H1` bar, together with its source and validation state.
...
Approval of `UK_EQUITIES_H1_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for United Kingdom Equities `H1` evidence.
...
- Twelve Data API documentation: `/time_series`, intervals `1h`, bounded date requests, ascending order, timezone selection, `/earliest_timestamp`, and documented maximum response size of 5,000 records.
