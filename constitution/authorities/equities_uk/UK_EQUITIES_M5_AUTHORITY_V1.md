...
This authority defines the approved operational truth for `M5` evidence within the United Kingdom Equities market ecosystem of Fragarach II.

It establishes:

- what one UK-equities `M5` bar represents;
...
- the `UK_EQUITIES_M5_VALIDATOR_V1` validation contract;
...
**Timeframe Code:** `M5`  
**Nominal Duration:** Five minutes, with an approved session-end shortened interval when required  
**Duration in Minutes:** `5 nominal`  
**Time Unit:** `MINUTE`  
**Interval Type:** `HYBRID_SESSION_ALIGNED`

## 5.2 Approved Definition

One canonical UK-equities `M5` bar represents a half-open interval inside the approved primary-venue regular trading session:

```text
[canonical interval open, canonical interval end)
```

Intervals begin at the official regular-session open and advance in exact 5-minute increments. The final interval of a session closes at the official regular-session close even when it is shorter than 5 minutes.
...
A complete UK-equities `M5` bar contains one approved source scope's regular-session OHLC and compatible volume for one registered security and one adjustment basis.

A bar MAY originate through `DIRECT_PROVIDER_M5`, `DIRECT_OPERATOR_M5`. No derived method is authorised in Version 1.
...
```text
expected_count = ceiling((official session close - official session open) / 5 minutes)
```
...
Intervals advance in exact 5-minute increments until the official session close.
...
- first bar: 08:00–08:05;
- final bar: 16:25–16:30;
- all 102 intervals are full five-minute intervals.

## 6.5 Official Early Closes

The same formula applies to an official early close. The final interval closes exactly at the approved session close and MAY be shorter than 5 minutes.
...
|---|---|---|---|---|
| Twelve Data `5min` | Provider intraday label proven to represent interval open | Request `Europe/London` | Validate against canonical session grid, then convert to UTC | Symbol, venue scope, session scope, adjustment basis, and effective range MUST validate |
...
A date without time cannot identify a `M5` interval.
...
Every canonical `M5` bar is owned by the approved primary venue's `Europe/London` trading date.
...
- `open` is the first eligible regular-session price in the governed `M5` interval;
...
## 10.2 Version 1 Construction Floor

M5 is the lowest authorised UK-equities timeframe in Version 1.

No lower-timeframe trades, quotes, ticks, or synthetic interpolation may be aggregated into M5 without a future approved authority.

## 10.3 Direct Evidence Requirements

Direct M5 evidence MUST declare security identity, provider, venue/consolidated scope, regular-session scope, listing currency, price-display unit, price basis, adjustment basis, volume scope, timestamp meaning, and effective range.

## 10.4 Forbidden Construction

Fragarach MUST NOT:

- combine trades or bars from different providers;
- merge venue-specific and consolidated scopes;
- mix continuous trading, auctions, off-book reports, or alternative sessions;
- mix adjusted and unadjusted evidence;
- interpolate missing prices;
- create flat bars for no-trade intervals;
- infer M5 from H1, M30, D1, quotes, or current price snapshots.
...
A shortened final interval ending at the official session close is complete at that close. It MUST NOT remain open until the nominal 5 nominal duration would have elapsed.

## 11.3 Latest Closed Bar

The latest closed `M5` bar is the greatest canonical interval open whose complete governed interval has closed and validated.
...
end_date   = E + 5 minutes represented in Europe/London
```
...
Every `M5` evidence lane MUST materialise an exact effective range for the registered security, provider symbol, primary venue, source scope, session scope, adjustment basis, listing currency, price-display unit, and provider interval.
...
The expected `M5` set consists of the intervals generated from the approved primary-venue regular-session schedule within the lane effective range.
...
- market code `EQUITIES_UK` and timeframe `M5`;
- primary venue, market segment or trading service, and calendar authority;
...
- construction method;
- effective range;
- validator `UK_EQUITIES_M5_VALIDATOR_V1`;
...
Current-As-Of Truth is the end instant or session date of the latest accepted closed `M5` bar, together with its source and validation state.
...
Approval of `UK_EQUITIES_M5_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for United Kingdom Equities `M5` evidence.
...
- Twelve Data API documentation: `/time_series`, intervals `5min`, bounded date requests, ascending order, timezone selection, `/earliest_timestamp`, and documented maximum response size of 5,000 records.
