...
**Authority Name:** `US_EQUITIES_D1_AUTHORITY_V1`  
**Market Name:** United States Equities  
**Market Code:** `EQUITIES_US`  
**Timeframe:** `D1`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/equities_us/US_EQUITIES_D1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `D1` evidence within the United States Equities market ecosystem of Fragarach II.

It establishes:

- what one US-equities `D1` bar represents;
...
- the `US_EQUITIES_D1_VALIDATOR_V1` validation contract;
...
One canonical US-equities `D1` bar represents one official primary-venue regular trading session for one registered security and one declared evidence scope.
...
A complete US-equities `D1` bar contains one approved source scope's regular-session OHLC and compatible volume for one registered security and one adjustment basis.
...
The owner date is the `America/New_York` civil date assigned by the approved primary-venue calendar.
...
Session boundaries MUST be resolved with historical IANA `America/New_York` timezone rules.
...
where `YYYY-MM-DD` is the approved primary-venue trading date in `America/New_York`.
...
Every canonical `D1` bar is owned by the approved primary venue's `America/New_York` trading date.
...
- session date;
- source provider and venue/consolidated scope;
- regular-session scope;
- currency;
- price basis;
- adjustment basis;
...
For an ordinary 09:30–16:00 session, a complete derived D1 bar requires:

```text
7 H1 session intervals, or
13 M30 intervals, or
78 M5 intervals
```

The H1 set includes the complete 15:30–16:00 session-end interval.
...
Where the registered source scope includes the official closing auction, the final contributing interval MUST include that auction according to the source's declared semantics.

A derived D1 bar that silently omits a required closing auction is incomplete.
...
end_date   = E + one calendar day represented in America/New_York
```
...
Every `D1` evidence lane MUST materialise an exact effective range for the registered security, provider symbol, primary venue, source scope, session scope, adjustment basis, and provider interval.
...
- market code `EQUITIES_US` and timeframe `D1`;
- primary venue and calendar authority;
...
- construction method;
- effective range;
- validator `US_EQUITIES_D1_VALIDATOR_V1`;
...
Approval of `US_EQUITIES_D1_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for United States Equities `D1` evidence.
