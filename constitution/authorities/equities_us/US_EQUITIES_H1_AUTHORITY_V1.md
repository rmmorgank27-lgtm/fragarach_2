...
**Authority Name:** `US_EQUITIES_H1_AUTHORITY_V1`  
**Market Name:** United States Equities  
**Market Code:** `EQUITIES_US`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/equities_us/US_EQUITIES_H1_AUTHORITY_V1.md`  
...
This authority defines the approved operational truth for `H1` evidence within the United States Equities market ecosystem of Fragarach II.

It establishes:

- what one US-equities `H1` bar represents;
...
- the `US_EQUITIES_H1_VALIDATOR_V1` validation contract;
...
One canonical US-equities `H1` bar represents a half-open interval inside the approved primary-venue regular trading session:
...
A complete US-equities `H1` bar contains one approved source scope's regular-session OHLC and compatible volume for one registered security and one adjustment basis.
...
```text
Expected H1 bars = 7, comprising six full 60-minute bars and one complete 30-minute session-end bar
```
...
- first bar: 09:30–10:30;
- sixth full bar: 14:30–15:30;
- final complete session-end bar: 15:30–16:00.
...
|---|---|---|---|---|
| Twelve Data `1h` | Provider intraday label proven to represent interval open | Request `America/New_York` | Validate against canonical session grid, then convert to UTC | Symbol, venue scope, session scope, adjustment basis, and effective range MUST validate |
...
Every canonical `H1` bar is owned by the approved primary venue's `America/New_York` trading date.
...
The ordinary final 15:30–16:00 session-end H1 interval requires:
...
Cross-provider, cross-venue-scope, cross-session, cross-adjustment, and extended-hours construction are prohibited.
...
Every `H1` evidence lane MUST materialise an exact effective range for the registered security, provider symbol, primary venue, source scope, session scope, adjustment basis, and provider interval.
...
- market code `EQUITIES_US` and timeframe `H1`;
- primary venue and calendar authority;
...
- construction method;
- effective range;
- validator `US_EQUITIES_H1_VALIDATOR_V1`;
...
Approval of `US_EQUITIES_H1_AUTHORITY_V1` means Fragarach II accepts this document as the constitutional source of truth for United States Equities `H1` evidence.
