# INDICES H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `INDICES_H1_AUTHORITY_V1`  
**Market:** Calculated Indices  
**Market Code:** `INDICES`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/indices/INDICES_H1_AUTHORITY_V1.md`  
...
This authority defines the constitutional operational truth for canonical Index `H1` evidence within Fragarach II.

It establishes:

- what one Index `H1` bar represents;
...
**Market:** `INDICES`  
**Timeframe:** `H1`  
**Nominal Duration:** 60 minutes within an approved index calculation window  
**Time Unit:** `HOUR`  
**Interval Type:** `HYBRID`  
**Validator:** `INDICES_H1_VALIDATOR_V1`
...
One canonical Index `H1` bar represents 60 minutes within an approved index calculation window for one registered calculated-index series under one approved evidence identity.
...
An index is eligible for `H1` only when its approved calculation frequency and publication profile can support that timeframe without interpolation.
...
Approved lower-timeframe construction may use M30 or M5 only under Section 10.
...
Every `H1` bar belongs to the calculation date whose registered calculation window contains its interval or whose official end-of-day publication owns the observation.
...
**Approved Construction Source:** `DIRECT PROVIDER BAR OR APPROVED LOWER-TIMEFRAME ROLLUP`
...
|---|---|---|---|
| `M30` | YES | Two complete M30 intervals with identical index and evidence identity | A profile-defined shortened session-end interval requires explicit treatment |
| `M5` | YES | Twelve complete M5 intervals with identical index and evidence identity | A profile-defined shortened session-end interval requires explicit treatment |
...
The expected grid is the exact profile-derived H1 grid inside the registered calculation window; no universal daily count exists.
...
Every Index `H1` Evidence Lane MUST bind:
...
Current-As-Of Truth is the latest complete profile-aligned H1 interval after accounting for calculation window, dissemination delay, and publication status.
...
- `INDICES_H1_AUTHORITY_V1` is approved;
...
- `INDICES_H1_VALIDATOR_V1` exists and is deterministic;
...
Approval means Fragarach II accepts this document as the constitutional authority for Index `H1` Evidence Lanes.
