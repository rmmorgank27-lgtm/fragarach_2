# INDICES M5 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `INDICES_M5_AUTHORITY_V1`  
**Market:** Calculated Indices  
**Market Code:** `INDICES`  
**Timeframe:** `M5`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/indices/INDICES_M5_AUTHORITY_V1.md`  
...
This authority defines the constitutional operational truth for canonical Index `M5` evidence within Fragarach II.

It establishes:

- what one Index `M5` bar represents;
...
**Market:** `INDICES`  
**Timeframe:** `M5`  
**Nominal Duration:** 5 minutes within an approved index calculation window  
**Time Unit:** `MINUTE`  
**Interval Type:** `HYBRID`  
**Validator:** `INDICES_M5_VALIDATOR_V1`
...
One canonical Index `M5` bar represents 5 minutes within an approved index calculation window for one registered calculated-index series under one approved evidence identity.
...
An index is eligible for `M5` only when its approved calculation frequency and publication profile can support that timeframe without interpolation.
...
No lower-timeframe rollup is authorised.
...
Every `M5` bar belongs to the calculation date whose registered calculation window contains its interval or whose official end-of-day publication owns the observation.
...
|---|---|---|---|
| None | NO | Version 1 does not construct M5 from another canonical timeframe | Raw or streaming observations require a future approved observation-stream authority |

## 10.4 Direct Versus Derived Precedence

Direct M5 evidence is the only approved canonical source in Version 1.

An official observation stream may become a direct M5 source only under a future approved observation-stream profile that defines sampling and aggregation.

## 10.5 Missing Source Bars

No lower-timeframe construction is authorised in Version 1.

A direct M5 bar missing required observation coverage is `INCOMPLETE` or `GAP`; it is not repaired by interpolation, proxy prices, futures, ETFs, CFDs, or constituent values.
...
The expected grid is the exact profile-derived M5 grid inside the registered calculation window; no universal daily count exists.
...
Every Index `M5` Evidence Lane MUST bind:
...
Current-As-Of Truth is the latest complete profile-aligned M5 interval after accounting for calculation window, dissemination delay, and publication status.
...
- `INDICES_M5_AUTHORITY_V1` is approved;
...
- `INDICES_M5_VALIDATOR_V1` exists and is deterministic;
...
Approval means Fragarach II accepts this document as the constitutional authority for Index `M5` Evidence Lanes.
