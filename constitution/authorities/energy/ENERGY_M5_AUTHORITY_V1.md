# ENERGY M5 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `ENERGY_M5_AUTHORITY_V1`  
**Market:** Energy Reference Prices  
**Market Code:** `ENERGY`  
**Timeframe:** `M5`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/energy/ENERGY_M5_AUTHORITY_V1.md`  
...
This authority defines the constitutional operational truth for canonical Energy `M5` evidence within Fragarach II.

It establishes:

- what one Energy `M5` bar represents;
...
**Validator:** `ENERGY_M5_VALIDATOR_V1`
...
One canonical Energy `M5` bar represents 5 minutes of one registered provider-derived Energy reference under the approved New York rollover profile.
...
The canonical grid is 288 nominal intervals per full owned day.
...
- direct evidence only under Version 1.
...
Intervals align from successive 5-minute intervals from each 17:00 America/New_York session open.

## 6.2 Boundary Rule

```text
[interval_open, interval_open + 5 minutes)
```
...
For session open instant `S` and integer `k >= 0`, canonical interval `k` is `[S + k×5 minutes, S + (k+1)×5 minutes)`, provided the interval is expected under the registered calendar profile.

## 6.4 Session Crossing

A canonical `M5` bar MUST NOT cross the 17:00 New York owned-day boundary.
...
The canonical timestamp is the UTC instant at which the 5-minute interval opens. `session_date` separately records the New York close-date owner.
...
Date-only values are invalid for Energy `M5` because they cannot identify one canonical intraday interval. They MUST be rejected with `DATE_ONLY_TIMESTAMP_UNSUPPORTED_FOR_INTRADAY`.
...
For direct evidence, OHLC retains the provider's declared `M5` aggregation.
...
Version 1 permits direct provider `M5` evidence only.

No lower timeframe is authorised to construct canonical Energy `M5` bars under this version.

Tick, `M1`, `M3`, or another finer source MAY be retained as evidence after separate authority exists, but MUST NOT be promoted into the canonical `M5` lane by this authority.


## 10.3 Direct Provider Authority

Twelve Data `5min` rows are approved as direct Energy `M5` evidence when:
...
- `ENERGY_M5_VALIDATOR_V1` accepts it.

Provider-side aggregation MUST remain identified as provider-side aggregation.

## 10.4 Derived Construction

Not applicable under Version 1. M5 construction from ticks or finer bars requires a separate authority.

## 10.5 Direct Versus Derived Precedence

Only direct provider or operator-supplied M5 evidence is eligible under Version 1. Comparable sources may coexist, but no source may silently overwrite another.
...
A logical Energy `M5` interval closes when its canonical interval end has passed.
...
The latest expected closed M5 interval is the greatest expected canonical interval whose end instant is less than or equal to the current instant.
...
```text
576 expected M5 intervals, equal to two full nominal Energy trading days
```
...
Each Energy `M5` lane MUST materialise:
...
There is no universal Energy `M5` start date.
...
| Day Type | Nominal `M5` Grid | Expected Rule |
|---|---:|---|
| Normal full owner day | 288 | All canonical intervals except approved maintenance exclusions |
...
| Public holiday without exception | 288 nominal | Holiday alone does not remove expectations |
...
A canonical Energy `M5` Evidence Lane key MUST include at least:
...
Freshness is measured against the latest expected closed Energy `M5` interval from Section 11.
...
| `DELAYED` | Latest accepted complete interval is one, two, or three expected M5 intervals behind | Continue with visible warning |
| `STALE` | Latest accepted complete interval is four or more expected M5 intervals behind | Continue with prominent warning and repair priority |
...
4. no derived source under Version 1.
...
- approved `ENERGY_M5_AUTHORITY_V1`;
...
Approval means Fragarach II accepts this document as the constitutional authority for Energy `M5` Evidence Lanes under the Version 1 provider-derived reference profile.
...
- Twelve Data documents `5min` as a supported time-series interval.
