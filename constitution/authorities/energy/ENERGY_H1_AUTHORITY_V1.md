# ENERGY H1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `ENERGY_H1_AUTHORITY_V1`  
**Market:** Energy Reference Prices  
**Market Code:** `ENERGY`  
**Timeframe:** `H1`  
**Version:** 1.0  
**Status:** APPROVED  
**Repository Location:** `constitution/authorities/energy/ENERGY_H1_AUTHORITY_V1.md`  
...
This authority defines the constitutional operational truth for canonical Energy `H1` evidence within Fragarach II.

It establishes:

- what one Energy `H1` bar represents;
...
**Validator:** `ENERGY_H1_VALIDATOR_V1`
...
One canonical Energy `H1` bar represents 60 minutes of one registered provider-derived Energy reference under the approved New York rollover profile.
...
The canonical grid is 24 nominal intervals per full owned day.
...
Intervals align from successive 60-minute intervals from each 17:00 America/New_York session open.

## 6.2 Boundary Rule

```text
[interval_open, interval_open + 60 minutes)
```
...
For session open instant `S` and integer `k >= 0`, canonical interval `k` is `[S + k×60 minutes, S + (k+1)×60 minutes)`, provided the interval is expected under the registered calendar profile.

## 6.4 Session Crossing

A canonical `H1` bar MUST NOT cross the 17:00 New York owned-day boundary.
...
The canonical timestamp is the UTC instant at which the 60-minute interval opens. `session_date` separately records the New York close-date owner.
...
Date-only values are invalid for Energy `H1` because they cannot identify one canonical intraday interval. They MUST be rejected with `DATE_ONLY_TIMESTAMP_UNSUPPORTED_FOR_INTRADAY`.
...
For direct evidence, OHLC retains the provider's declared `H1` aggregation.
...
|---|---|---|
| `M30` | YES, CONDITIONAL | Approved `ENERGY_M30_AUTHORITY_V1`; same instrument, source nature, source scope, price basis, pricing unit, roll methodology; 2 expected M30 bars or profile-adjusted complete coverage |
| `M5` | YES, CONDITIONAL | Approved `ENERGY_M5_AUTHORITY_V1`; same instrument, source nature, source scope, price basis, pricing unit, roll methodology; 12 expected M5 bars or profile-adjusted complete coverage |

All other source timeframes are prohibited under Version 1 unless separately authorised.

## 10.3 Direct Provider Authority

Twelve Data `1h` rows are approved as direct Energy `H1` evidence when:
...
A logical Energy `H1` interval closes when its canonical interval end has passed.
...
The latest expected closed H1 interval is the greatest expected canonical interval whose end instant is less than or equal to the current instant.
...
```text
48 expected H1 intervals, equal to two full nominal Energy trading days
```
...
Each Energy `H1` lane MUST materialise:
...
There is no universal Energy `H1` start date.
...
| Day Type | Nominal `H1` Grid | Expected Rule |
|---|---:|---|
| Normal full owner day | 24 | All canonical intervals except approved maintenance exclusions |
...
| Public holiday without exception | 24 nominal | Holiday alone does not remove expectations |
...
A canonical Energy `H1` Evidence Lane key MUST include at least:
...
Freshness is measured against the latest expected closed Energy `H1` interval from Section 11.
...
| `DELAYED` | Latest accepted complete interval is exactly one expected H1 interval behind | Continue with visible warning |
| `STALE` | Latest accepted complete interval is two or more expected H1 intervals behind | Continue with prominent warning and repair priority |
...
- approved `ENERGY_H1_AUTHORITY_V1`;
...
Approval means Fragarach II accepts this document as the constitutional authority for Energy `H1` Evidence Lanes under the Version 1 provider-derived reference profile.
...
- Twelve Data documents `1h` as a supported time-series interval.
