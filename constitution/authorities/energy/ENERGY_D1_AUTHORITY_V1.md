# ENERGY D1 AUTHORITY V1

**Document Class:** Constitutional Timeframe Authority  
**Authority Layer:** Market Timeframe  
**Authority Name:** `ENERGY_D1_AUTHORITY_V1`  
**Market:** Energy Reference Prices  
**Market Code:** `ENERGY`  
**Timeframe:** `D1`  
**Version:** 1.0  
**Status:** DRAFT FOR APPROVAL  
**Repository Location:** `constitution/authorities/energy/ENERGY_D1_AUTHORITY_V1.md`  
...
This authority defines the constitutional operational truth for canonical Energy `D1` evidence within Fragarach II.

It establishes:

- what one Energy `D1` bar represents;
...
One canonical Energy `D1` bar represents one owned Energy trading day of one registered provider-derived Energy reference under the approved New York rollover profile.
...
The canonical grid is one session-derived bar.
...
Intervals align from the 17:00 America/New_York daily boundary.

## 6.2 Boundary Rule

```text
[previous owner boundary at 17:00 New York, owner boundary at 17:00 New York)
```
...
For owner date `D`, let `S(D)` be 17:00 America/New_York on the preceding civil date. The one canonical D1 interval is `[S(D), S(D)+24 hours)` and is owned by `D`.

## 6.4 Session Crossing

A canonical `D1` bar MUST NOT cross the 17:00 New York owned-day boundary.
...
An early termination or provider-shortened daily bar is not complete unless an approved exceptional-closure record changes the canonical interval. A direct provider daily row may be retained as `PARTIAL` or `PROVISIONAL`.
...
**Timestamp Meaning:** `SEMANTIC OWNER-DATE LABEL`  
**Canonical Storage Timezone:** `UTC`

The canonical stored timestamp is `YYYY-MM-DDT00:00:00Z`, where the date is the owned New York close date. It is a semantic date label and MUST NOT be interpreted as the physical interval open.

## 7.2 Provider Timestamp Mapping

| Provider | Provider Timestamp Meaning | Requested Timezone | Canonical Mapping | Notes |
|---|---|---|---|---|
| Twelve Data | Date or daily timestamp representing provider `1day` row | Requested `America/New_York` | A provider date or midnight timestamp is mapped to the same New York owner date after provider-symbol proof. The canonical stored timestamp is then the semantic UTC date label. | Mapping proof required per registered symbol |
...
Date-only values are valid only for direct D1 evidence when the date maps to the owned New York close date. Canonical storage uses midnight UTC as a semantic label. Date-only input MUST NOT be interpreted as a UTC-midnight trading interval.
...
For direct evidence, OHLC retains the provider's declared `D1` aggregation.
...
|---|---|---|
| `H1` | YES, CONDITIONAL | Approved `ENERGY_H1_AUTHORITY_V1`; same instrument, source nature, source scope, price basis, pricing unit, roll methodology; 24 expected H1 bars or profile-adjusted complete coverage |
| `M30` | YES, CONDITIONAL | Approved `ENERGY_M30_AUTHORITY_V1`; same instrument, source nature, source scope, price basis, pricing unit, roll methodology; 48 expected M30 bars or profile-adjusted complete coverage |
| `M5` | YES, CONDITIONAL | Approved `ENERGY_M5_AUTHORITY_V1`; same instrument, source nature, source scope, price basis, pricing unit, roll methodology; 288 expected M5 bars or profile-adjusted complete coverage |

All other source timeframes are prohibited under Version 1 unless separately authorised.

## 10.3 Direct Provider Authority

Twelve Data `1day` rows are approved as direct Energy `D1` evidence when:
...
A logical Energy `D1` interval closes when its canonical interval end has passed.
...
The latest expected closed owner date is the greatest Monday-through-Friday New York civil date whose 17:00 New York interval end is less than or equal to the current instant, excluding only approved exceptional non-bar dates.
...
For inclusive owner-date range `[S, E]`, request `start_date = S 00:00:00 America/New_York` and `end_date = E+1 civil day 00:00:00 America/New_York`, then filter by mapped owner date.
...
Each Energy `D1` lane MUST materialise:
...
There is no universal Energy `D1` start date.
...
| Day Type | Expected D1 Bars | Rule |
|---|---:|---|
| Normal Monday-Friday owner date | 1 | One closed session-derived D1 bar |
| Saturday or Sunday owner date | 0 | Weekend closure |
| Approved exceptional non-bar date | 0 | Requires explicit exception evidence |
| Public holiday without exception | 1 | Holiday alone does not remove expectation |
...
A canonical Energy `D1` Evidence Lane key MUST include at least:
...
Freshness is measured against the latest expected closed Energy `D1` interval from Section 11.
...
| `DELAYED` | Latest accepted complete interval is exactly one expected D1 bar behind | Continue with visible warning |
| `STALE` | Latest accepted complete interval is two or more expected D1 bars behind | Continue with prominent warning and repair priority |
...
- approved `ENERGY_D1_AUTHORITY_V1`;
...
Approval means Fragarach II accepts this document as the constitutional authority for Energy `D1` Evidence Lanes under the Version 1 provider-derived reference profile.
