...
REQUEST_STATES = frozenset(
    {"REQUESTED", "ACCEPTED", "TRANSFERRING", "VERIFYING", "ACTIVE",
...
def submit_request(paths: ReplicaPaths, client_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    request = {
        ...
        "timeframe": timeframe, "state": "ACCEPTED", "attempt": 1,
        "requested_at_utc": str(payload.get("requested_at_utc") or now),
        "accepted_at_utc": now, "updated_at_utc": now,
        "artifact_id": artifact["artifact_id"], "source_revision": artifact["source_revision"],
        "caodt": artifact["caodt"], "lane_fingerprint": artifact["lane_fingerprint"],
        "expected_bytes": int(artifact["payload"]["bytes"]),
