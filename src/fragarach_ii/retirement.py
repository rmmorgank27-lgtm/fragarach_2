...
def retirement_impact(database_path:str|Path,asset:str,scope="WHOLE_INSTRUMENT",selected_lanes:tuple[str,...]=()):
    ...
    try:
        ...
        all_lanes=tuple(r[0] for r in connection.execute("SELECT timeframe FROM evidence_lanes WHERE asset=? ORDER BY timeframe",(symbol,)).fetchall());lanes=all_lanes if scope=="WHOLE_INSTRUMENT" else tuple(sorted(set(x.upper() for x in selected_lanes)))
