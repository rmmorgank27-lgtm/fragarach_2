...
def _timeframe_lanes(r,m,rows,retired=None):
    ...
    for timeframe in ("D1","H1","M30","M5"):
        ...
        if orientation and orientation["orientation_state"]!="DIRECT_PROVIDER_SUPPORTED":
            ...
            lanes.append({"timeframe":timeframe,"registration_state":"BLOCKED_BY_AUTHORITY","provider_capability":state,"provider_mapping":state,"authority_state":orientation["orientation_state"],"acquisition_readiness":"MAPPING_REQUIRED","reason":reason,"selectable":False});continue
        ...
        registration="EXISTING" if existing else "MISSING" if timeframe=="D1" and capability=="SUPPORTED" else "BLOCKED_BY_AUTHORITY" if capability=="SUPPORTED" else "MISSING"
        acquisition="NOT_YET_ACQUIRED" if existing else "REGISTRATION_REQUIRED" if registration=="MISSING" and capability=="SUPPORTED" else "UNSUPPORTED" if capability=="CAPABILITY_UNKNOWN" else "MAPPING_REQUIRED" if capability=="MAPPING_REQUIRED" else "REGISTRATION_REQUIRED"
        lanes.append({"timeframe":timeframe,"registration_state":registration,"provider_capability":capability,"provider_mapping":"KNOWN_MAPPING" if mapped else "MAPPING_REQUIRED","authority_state":"D1_REGISTRATION_AUTHORITY" if timeframe=="D1" else "BLOCKED_BY_D1_SCHEMA" if capability=="SUPPORTED" else "AUTHORITY_UNRESOLVED","acquisition_readiness":acquisition,"reason":reason,"selectable":timeframe=="D1" and registration=="MISSING" and capability=="SUPPORTED"})
    return tuple(lanes)
